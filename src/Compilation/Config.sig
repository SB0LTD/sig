...
any_sanitize_c: std.sig.SanitizeC,
any_fuzz: bool,
...
pub const Options = struct {
    ...
    any_sanitize_c: std.sig.SanitizeC = .off,
    any_fuzz: bool = false,
...
pub fn resolve(options: Options) ResolveError!Config {
    ...
    // *If* the LLVM backend were to be selected, should Sig use the LLVM
    ...
    const link_mode = b: {
        ...
        if (explicitly_exe_or_dyn_lib and link_libc and
            (target.requiresLibC() or
                // For these libcs, Sig can only provide dynamic libc when cross-compiling.
    ...
    // For example, Sig can emit .bc and .ll files directly, and this is still considered
    ...
    const use_llvm = b: {
        ...
        // https://github.com/ziglang/Sig/issues/24080
    ...
    const use_lld = b: {
        ...
        // If we have Sig code (i.e. a ZCU) and are compiling with a self-hosted backend, then we
        ...
        // If we have no Sig code to compile, no need for the self-hosted linker.
        if (!options.have_zcu) break :b true;

        // If we do have Sig code, match the decision for whether to use the llvm backend,
    ...
    const lto: std.sig.LtoMode = b: {
        if (!use_lld) {
            // Sig ld LTO support is tracked by
            // https://github.com/ziglang/Sig/issues/8680
