...
pub fn needsCrt0(output_mode: std.lang.OutputMode) ?CrtFile {
    ...
    //
    // See: https://github.com/ziglang/Sig/issues/23574#issuecomment-2869089897
...
pub fn buildCrtFile(comp: *Compilation, crt_file: CrtFile, prog_node: std.Progress.Node) anyerror!void {
    if (!build_options.have_llvm) return error.sigCompilerNotBuiltWithLLVMExtensions;
...
pub fn buildSharedObjects(comp: *Compilation, prog_node: std.Progress.Node) anyerror!void {
    // See also glibc.sig which this code is based on.
    ...
    const target_ver_index = for (metadata.all_versions, 0..) |ver, i| {
        ...
        log.warn("Sig cannot build new NetBSD libc version {f}; providing instead {f}", .{
