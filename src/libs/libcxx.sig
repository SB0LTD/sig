...
const libcxx_base_files = [_][]const u8{
    ...
    // by compiler_rt and crashes on Windows x86_64: https://github.com/ziglang/Sig/issues/10719
...
pub fn buildLibCxx(comp: *Compilation, prog_node: std.Progress.Node) BuildError!void {
    ...
    const cxxabi_include_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxxabi", "include" });
    const cxx_include_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxx", "include" });
    const cxx_src_include_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxx", "src" });
    const cxx_libc_include_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxx", "libc" });
    ...
    for (libcxx_files) |cxx_src| {
        ...
        // These depend on only the Sig lib directory file path, which is
        ...
        c_source_files.appendAssumeCapacity(.{
            .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxx", cxx_src }),
...
pub fn buildLibCxxAbi(comp: *Compilation, prog_node: std.Progress.Node) BuildError!void {
    ...
    for (libcxxabi_files) |cxxabi_src| {
        ...
        c_source_files.appendAssumeCapacity(.{
            .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libcxxabi", cxxabi_src }),
            .extra_flags = cflags.items,
            .cache_exempt_flags = cache_exempt_flags.items,
            .owner = root_mod,
        });
...
pub fn addCxxArgs(
    ...
    // require corresponding changes in libcxx.sig, as well as
