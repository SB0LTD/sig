...
const def = @import("mingw/def.sig");
const implib = @import("mingw/implib.sig");

const Preprocessor = @import("mingw/Preprocessor.sig");
    ...
    switch (crt_file) {
        .crt2_o => {
            ...
            var files = [_]Compilation.CSourceFile{
                .{
                    .src_path = try comp.dirs.sig_lib.join(arena, &.{
            ...
            return comp.build_crt_file("crt2", .Obj, .@"mingw-w64 crt2.o", prog_node, &files, .{
                .function_sections = false, // https://codeberg.org/ziglang/Sig/issues/30702
            });
        },
...
fn addCcArgs(
    ...
    try args.appendSlice(&[_][]const u8{
        ...
        "-isystem",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "include", "any-windows-any" }),
    });
}
...
fn addCrtCcArgs(
    ...
    try args.appendSlice(&[_][]const u8{
        ...
        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "mingw", "include" }),
    });
}
...
pub fn buildImportLib(comp: *Compilation, lib_name: []const u8, prog_node: std.Progress.Node) !Cache.Path {
    ...
    const def_file_path: Cache.Path = .{
        ...
        .sub_path = findDef(arena, io, comp.getTarget(), comp.dirs.sig_lib, lib_name) catch |err| switch (err) {
    ...
    cache.hash.addOptionalBytes(comp.dirs.sig_lib.path);
...
pub fn libExists(
    ...
    const s = findDef(allocator, io, target, SIG_LIB_DIRectory, lib_name) catch |err| switch (err) {
...
fn findDef(
...

    {
        ...
        if (SIG_LIB_DIRectory.handle.access(io, override_path.items, .{})) |_| {
