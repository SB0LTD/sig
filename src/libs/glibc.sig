...
pub const ABI = struct {
    ...
    all_targets: []const std.sig.target.ArchOsAbi,
...
pub const LoadMetaDataError = error{
    /// The files that ship with the Sig compiler were unable to be read, or otherwise had malformed data.
...
pub fn loadMetaData(gpa: Allocator, contents: []const u8) LoadMetaDataError!*ABI {
    ...
    const targets = b: {
        ...
        const targets = try arena.alloc(std.sig.target.ArchOsAbi, targets_len);
        var i: u8 = 0;
...
fn start_asm_path(comp: *Compilation, arena: Allocator, basename: []const u8) ![]const u8 {
    ...
    try result.appendSlice(comp.dirs.sig_lib.path orelse ".");
...
fn add_include_dirs(comp: *Compilation, arena: Allocator, args: *std.array_list.Managed([]const u8)) error{OutOfMemory}!void {
    ...
    if (opt_nptl) |nptl| {
        try args.append("-I");
        try args.append(try path.join(arena, &.{ comp.dirs.sig_lib.path orelse ".", lib_libc_glibc ++ "sysdeps", nptl }));
    }
    ...
    try args.append("-I");
    try args.append(try path.join(arena, &[_][]const u8{ comp.dirs.sig_lib.path orelse ".", lib_libc ++ "glibc" }));

    try args.append("-I");
    try args.append(try std.fmt.allocPrint(arena, "{s}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "{s}-{s}-{s}", .{
        comp.dirs.sig_lib.path orelse ".",
        std.sig.target.glibcArchNameHeaders(target.cpu.arch),
        @tagName(target.os.tag),
        std.sig.target.glibcAbiNameHeaders(target.abi),
    }));

    try args.append("-I");
    try args.append(try lib_path(comp, arena, lib_libc ++ "include" ++ s ++ "generic-glibc"));

    const arch_name = std.sig.target.osArchName(target);
    try args.append("-I");
    try args.append(try std.fmt.allocPrint(arena, "{s}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "{s}-linux-any", .{
        comp.dirs.sig_lib.path orelse ".", arch_name,
    }));
...
fn add_include_dirs_arch(
    ...
    if (is_x86) {
        ...
        if (opt_nptl) |nptl| {
            ...
            // coldfire ABI support requires: https://github.com/ziglang/Sig/issues/20690
            try args.append("-I");
...
fn lib_path(comp: *Compilation, arena: Allocator, sub_path: []const u8) ![]const u8 {
    return path.join(arena, &.{ comp.dirs.sig_lib.path orelse ".", sub_path });
}
...
pub fn buildSharedObjects(comp: *Compilation, prog_node: std.Progress.Node) anyerror!void {
    ...
    cache.addPrefix(comp.dirs.sig_lib);
    ...
    const target_targ_index = for (metadata.all_targets, 0..) |targ, i| {
        ...
        unreachable; // std.sig.target.available_libcs prevents us from getting here
    };
    ...
    const target_ver_index = for (metadata.all_versions, 0..) |ver, i| {
        ...
        log.warn("Sig cannot build new glibc version {f}; providing instead {f}", .{
