...
pub fn buildCrtFile(comp: *Compilation, in_crt_file: CrtFile, prog_node: std.Progress.Node) anyerror!void {
    ...
    switch (in_crt_file) {
        ...
        .libc_a => {
            ...
            const arch_name = std.sig.target.muslArchName(target.cpu.arch, target.abi);
            ...
            while (it.next()) |entry| {
                ...
                c_source_file.* = .{
                    .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libc", src_file }),
                    .extra_flags = args.items,
                    .owner = undefined,
                };
            }
        ...
        .libc_so => {
            ...
            const sub_compilation = Compilation.create(comp.gpa, arena, io, &sub_create_diag, .{
                ...
                .c_source_files = &.{
                    .{
                        .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "libc.S" }),
                        .owner = root_mod,
                    },
                },
...
fn addCcArgs(
    ...
    try args.appendSlice(&[_][]const u8{
        ...
        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "arch", arch_name }),

        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "arch", "generic" }),

        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "src", "include" }),

        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "src", "internal" }),

        "-I",
        try comp.dirs.sig_lib.join(arena, &.{ "libc", "musl", "include" }),
...
fn start_asm_path(comp: *Compilation, arena: Allocator, basename: []const u8) ![]const u8 {
    const target = comp.getTarget();
    return comp.dirs.sig_lib.join(arena, &.{
        "libc", "musl", "crt", std.sig.target.muslArchName(target.cpu.arch, target.abi), basename,
    });
}
