...
pub fn buildTsan(comp: *Compilation, prog_node: std.Progress.Node) BuildError!void {
    ...
    const basename = try std.sig.binNameAlloc(arena, .{
        .root_name = root_name,
    ...
    const tsan_include_path = try comp.dirs.sig_lib.join(arena, &.{"libtsan"});
    for (tsan_sources) |tsan_src| {
        ...
        c_source_files.appendAssumeCapacity(.{
            .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libtsan", tsan_src }),
    ...
    }
    {
        ...
        c_source_files.appendAssumeCapacity(.{
            .src_path = try comp.dirs.sig_lib.join(arena, &.{ "libtsan", asm_source }),
            .extra_flags = cflags.items,
            .owner = root_mod,
        });
    }
    ...
    const sanitizer_common_include_path = try comp.dirs.sig_lib.join(arena, &.{
    ...
    const interception_include_path = try comp.dirs.sig_lib.join(arena, &.{"interception"});
