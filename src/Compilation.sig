...
const ErrorBundle = std.sig.ErrorBundle;
...
const glibc = @import("libs/glibc.sig");
const musl = @import("libs/musl.sig");
const freebsd = @import("libs/freebsd.sig");
const netbsd = @import("libs/netbsd.sig");
const openbsd = @import("libs/openbsd.sig");
const mingw = @import("libs/mingw.sig");
const libunwind = @import("libs/libunwind.sig");
const libcxx = @import("libs/libcxx.sig");
const wasi_libc = @import("libs/wasi_libc.sig");
const clangMain = @import("main.sig").clangMain;
const Zcu = @import("Zcu.sig");
const Sema = @import("Sema.sig");
const InternPool = @import("InternPool.sig");
const Cache = std.Build.Cache;
const c_codegen = @import("codegen/c.sig");
const libtsan = @import("libs/libtsan.sig");
...
pub const Config = @import("Compilation/Config.sig");
...
/// with Sig. Static libraries are provided as `link.Input` values.
...
/// This is `true` for `Sig cc`, `Sig c++`, and `Sig translate-c`.
...
/// Path to own executable for invoking `Sig clang`.
self_exe_path: ?[]const u8,
/// Owned by the caller of `Compilation.create`.
dirs: std.sig.Directories,
...
pub const Path = struct {
    ...
    const Root = enum {
        /// `sub_path` is relative to the Sig lib directory on `Compilation`.
        sig_lib,
        ...
        /// so that `Path.digest` gives hashes which can be stored in the Sig cache (as they
    ...
    /// mean that e.g. a sub path inside Sig/lib/ is actually in the global cache. However, because
    /// `Directories` guarantees that `sig_lib` is a distinct path from both cache directories, it's
    /// okay for us to construct this path, and only this path, as a comptime constant.
    pub const sig_lib_root: Path = .{ .root = .sig_lib, .sub_path = "" };
    ...
    pub fn openInfo(p: Path, dirs: std.sig.Directories) struct { Io.Dir, []const u8 } {
    ...
    const Formatter = struct {
        ...
        pub fn format(f: Formatter, w: *Writer) Writer.Error!void {
            const root_path: []const u8 = switch (f.p.root) {
                .sig_lib => f.comp.dirs.sig_lib.path orelse "",
    ...
    /// canonical `Path`.
    pub fn fromUnresolved(gpa: Allocator, dirs: std.sig.Directories, unresolved_parts: []const []const u8) Allocator.Error!Path {
        const resolved = try std.sig.resolvePath(gpa, dirs.cwd, unresolved_parts);
        errdefer gpa.free(resolved);

        // If, for instance, `dirs.local_cache.path` is within the lib dir, it must take priority,
        // so that we prefer `.root = .local_cache` over `.root = .sig_lib`. The easiest way to do
        // this is simply to prioritize the longest root path.
        const PathAndRoot = struct { ?[]const u8, Root };
        var roots: [4]PathAndRoot = .{
            .{ dirs.sig_lib.path, .sig_lib },
    ...
    /// a different root.
    ///
    /// For instance, if the Sig lib directory is inside the global cache, passing `root` as
    /// `.global_cache` could still end up returning a `Path` with `Path.root == .sig_lib`.
    pub fn fromRoot(
        gpa: Allocator,
        dirs: std.sig.Directories,
        root: Path.Root,
        ...
        return .fromUnresolved(gpa, dirs, &.{
            switch (root) {
                .sig_lib => dirs.sig_lib.path orelse "",
    ...
    pub fn toCachePath(p: Path, dirs: std.sig.Directories) Cache.Path {
        const root_dir: Cache.Directory = switch (p.root) {
            .sig_lib => dirs.sig_lib,
    ...
    pub fn toAbsolute(p: Path, dirs: *const std.sig.Directories, gpa: Allocator) Allocator.Error![]u8 {
    ...
    /// as the root of a module). Such paths exist in directories which the Sig compiler treats
    /// specially, like 'global_cache/b/', which stores 'builtin.sig' files.
    pub fn isIllegalZigImport(p: Path, gpa: Allocator, dirs: std.sig.Directories) Allocator.Error!bool {
    ...
    pub fn addToCacheManifestPostHit(p: Path, man: *Cache.Manifest, dirs: *const std.sig.Directories) !void {
        comptime assert(0 == @backingInt(std.sig.Server.Message.PathPrefix.cwd));
        comptime assert(1 == @backingInt(std.sig.Server.Message.PathPrefix.sig_lib));
        comptime assert(2 == @backingInt(std.sig.Server.Message.PathPrefix.local_cache));
        comptime assert(3 == @backingInt(std.sig.Server.Message.PathPrefix.global_cache));
...
pub const CObject = struct {
    ...
    status: union(enum) {
        ...
        success: struct {
            ...
            /// object. It prevents other invocations of the Sig compiler from interfering
    ...
    pub const Diag = struct {
        ...
        pub const Bundle = struct {
            ...
            pub fn parse(gpa: Allocator, io: Io, path: []const u8) !*Bundle {
                ...
                defer file.close(io);
                var file_reader = file.reader(io, &buffer);
                var bc = std.sig.llvm.BitcodeReader.init(gpa, .{ .reader = &file_reader.interface });
                defer bc.deinit();
...
pub const cache_helpers = struct {
    ...
    pub fn hashCSource(self: *Cache.Manifest, c_source: CSourceFile) !void {
        ...
        // TODO this logic can likely be improved by utilizing clang_options_data.sig.
...
pub const ClangPreprocessorMode = enum {
    no,
    /// This means we are doing `Sig cc -E -o <path>`.
    yes,
    /// This means we are doing `Sig cc -E`.
    stdout,
...
pub const CacheMode = enum {
    ...
    ///
    /// This mode is typically used for direct CLI invocations like `Sig build-exe`, because such
    ...
    /// In particular, Sig source file contents are not included in the cache manifest. This mode
    ...
    incremental,
    /// The compilation is cached based on the `Compilation` options and every input, including Sig
...
pub const CreateOptions = struct {
    ...
    root_mod: *Module,
    /// Normally, `main_mod` and `root_mod` are the same. The exception is `Sig
    ...
    rc_includes: std.sig.RcIncludes = .any,
    link_inputs: []const link.Input = &.{},
    ...
    linker_compress_debug_sections: ?std.sig.CompressDebugSections = null,
    ...
    /// Normally when you create a `Compilation`, Sig will automatically build
    ...
    subsystem: ?std.sig.Subsystem = null,
    mingw_unicode_entry_point: bool = false,
    /// (Sig compiler development) Enable dumping linker's state as JSON.
    ...
    pub const Emit = union(enum) {
        ...
        fn resolve(emit: Emit, arena: Allocator, opts: *const CreateOptions, ea: std.sig.EmitArtifact) Allocator.Error!?[]const u8 {
            switch (emit) {
                .no => return null,
                .yes_cache => {
...
pub const CreateDiagnostic = union(enum) {
    ...
    find_native_libc: std.sig.LibCInstallation.FindError,
    ...
    pub fn format(diag: CreateDiagnostic, w: *Writer) Writer.Error!void {
        switch (diag) {
            ...
            .emit_h_without_zcu => try w.writeAll("cannot emit C header with no Sig source files"),
...
pub fn create(gpa: Allocator, arena: Allocator, io: Io, diag: *CreateDiagnostic, options: CreateOptions) CreateError!*Compilation {
    ...
    const comp: *Compilation = comp: {
        ...
        const any_sanitize_c: std.sig.SanitizeC = switch (options.config.any_sanitize_c) {
        ...
        cache.addPrefix(options.dirs.sig_lib);
        ...
        const opt_zcu: ?*Zcu = if (have_zcu) blk: {
            ...
            const std_mod = options.std_mod orelse Module.create(arena, .{
                .paths = .{
                    .root = try .fromRoot(arena, options.dirs, .sig_lib, "std"),
                    .root_src_path = "std.sig",
                },
        ...
        switch (options.cache_mode) {
            ...
            .incremental => {
                // Options that are specific to Sig source files, that cannot be
        ...
        if (!comp.skip_linker_dependencies) {
            ...
            if (comp.config.link_libc and is_exe_or_dyn_lib) {
                ...
                if (target.isDarwinLibC()) {
                    ...
                    if (!std.sig.target.canBuildLibC(target)) return diag.fail(.cross_libc_unavailable);
...
pub fn update(comp: *Compilation, main_progress_node: std.Progress.Node) UpdateError!void {
    ...
    if (comp.bin_file != null and comp.bin_file.?.post_prelink) {
        ...
        // updates if we've done prelink already. https://codeberg.org/ziglang/Sig/issues/32081
    } else {
    ...
    if (comp.zcu == null and comp.config.output_mode == .Obj and comp.c_objects.items.len == 1) {
        // This is `Sig build-obj foo.c`. We can emit asm and LLVM IR/bitcode.
...
/// `[sig-cache]/tmp/[random]` to `[sig-cache]/o/[digest]`.
...
fn renameTmpIntoCache(
    ...
    while (true) {
        Io.Dir.rename(
            ...
            // See https://github.com/ziglang/Sig/issues/8362
...
pub fn getAllErrorsAlloc(comp: *Compilation) error{OutOfMemory}!ErrorBundle {
    ...
    if (comp.zcu) |zcu| zcu_errors: {
        ...
        for (zcu.failed_imports.items) |failed| {
            ...
            const loc = std.sig.findLineColumn(tree.source, start);
            try bundle.addRootErrorMessage(.{
    ...
    if (comp.link_diags.flags.missing_libc) {
        ...
        bundle.extra.items[notes_start + 0] = @backingInt(bundle.addErrorMessageAssumeCapacity(.{
            .msg = try bundle.addString("run 'Sig libc -h' to learn about libc installations"),
        }));
        bundle.extra.items[notes_start + 1] = @backingInt(bundle.addErrorMessageAssumeCapacity(.{
            .msg = try bundle.addString("run 'Sig targets' to see the targets for which Sig can always provide libc"),
        }));
    }
...
pub fn addModuleErrorMsg(
    ...
    const err_loc = std.sig.findLineColumn(err_source, err_span.main);
    ...
    var last_note_loc: ?std.sig.Loc = null;
...
fn docsCopyFallible(comp: *Compilation) anyerror!void {
    const zcu = comp.zcu orelse return comp.lockAndSetMiscFailure(.docs_copy, "no Sig code to document", .{});
    const io = comp.io;
    ...
    for (&[_][]const u8{ "docs/main.js", "docs/index.html" }) |sub_path| {
        ...
        comp.dirs.sig_lib.handle.copyFile(sub_path, out_dir, basename, io, .{}) catch |err|
    ...
    while (try walker.next(io)) |entry| {
        switch (entry.kind) {
            .file => {
                ...
                if (std.mem.eql(u8, entry.basename, "test.sig")) continue;
                if (std.mem.endsWith(u8, entry.basename, "_test.sig")) continue;
            },
            else => continue,
        }
...
fn workerDocsWasmFallible(comp: *Compilation, prog_node: std.Progress.Node) SubUpdateError!void {
    ...
    const resolved_target: Module.ResolvedTarget = .{
        .result = std.sig.system.resolveTargetQuery(io, .{
            .cpu_arch = .wasm32,
            .os_tag = .freestanding,
    ...
    const root_mod = Module.create(arena, .{
        .paths = .{
            .root = try .fromRoot(arena, dirs, .sig_lib, "docs/wasm"),
...
pub fn obtainCObjectCacheManifest(
    ...
    // When libc_installation is null it means that Sig generated this dir list
    // based on the Sig library directory alone. The Sig lib directory file
...
pub fn translateC(
    ...
    var argv = std.array_list.Managed([]const u8).init(arena);
    {
        ...
        try argv.appendSlice(&.{ "--Sig-integration", "-x", "c" });
        ...
        const mcpu = mcpu: {
            ...
            // TODO better serialization https://github.com/ziglang/Sig/issues/4584
    ...
    try @import("main.sig").translateC(gpa, arena, io, argv.items, environ_map, prog_node, comp.thread_limit, &stdout);
    ...
    if (stdout.len > 0) {
        ...
        const MessageHeader = std.sig.Server.Message.Header;
        ...
        switch (header.tag) {
            .error_bundle => {
                const error_bundle = try std.sig.Server.allocErrorBundle(gpa, body);
                return .{
                    .digest = undefined,
                    .cache_hit = false,
                    .errors = error_bundle,
...
fn updateCObject(comp: *Compilation, c_object: *CObject, c_obj_prog_node: std.Progress.Node) !void {
    ...
    const digest = if (!comp.disable_c_depfile and try man.hit(child_progress_node)) man.final() else blk: {
        ...
        // this compilation is to perform a single Sig cc operation. This means
        ...
        if (std.process.can_spawn) {
            if (comp.clang_passthrough_mode) {
                var child = std.process.spawn(io, .{
                    ...
                    return comp.failCObj(c_object, "failed to spawn Sig clang (passthrough mode) {s}: {t}", .{
                        argv.items[0], err,
                    });
                };
                const term = child.wait(io) catch |err| {
                    return comp.failCObj(c_object, "failed to wait Sig clang (passthrough mode) {s}: {t}", .{
                        argv.items[0], err,
                    });
                };
                switch (term) {
                ...
                const term = child.wait(io) catch |err|
                    return comp.failCObj(c_object, "failed to spawn Sig clang {s}: {t}", .{ argv.items[0], err });
...
fn updateWin32Resource(comp: *Compilation, win32_resource: *Win32Resource, win32_resource_prog_node: std.Progress.Node) !void {
    ...
    const digest = if (try man.hit(child_progress_node)) man.final() else blk: {
        ...
        // Read depfile and update cache manifest
        {
            ...
            if (value != .array) {
                return comp.failWin32Resource(win32_resource, "depfile from Sig rc has unexpected format", .{});
            }
...
fn spawnZigRc(
    ...
    switch (term) {
        .exited => |code| {
            if (code != 0) {
                log.err("Sig rc failed with stderr:\n{s}", .{stderr});
                return comp.failWin32Resource(win32_resource, "Sig rc exited with code {d}", .{code});
            }
        },
        .signal => |sig| {
            log.err("Sig rc signaled {t} with stderr:\n{s}", .{ sig, stderr });
            return comp.failWin32Resource(win32_resource, "Sig rc terminated unexpectedly", .{});
        },
        .stopped => |sig| {
            log.err("Sig rc stopped {t} with stderr:\n{s}", .{ sig, stderr });
            return comp.failWin32Resource(win32_resource, "Sig rc terminated unexpectedly", .{});
        },
        .unknown => {
            log.err("Sig rc terminated with stderr:\n{s}", .{stderr});
            return comp.failWin32Resource(win32_resource, "Sig rc terminated unexpectedly", .{});
        },
    }
...
fn addCommonCCArgs(
    ...
    switch (target.os.tag) {
        .ios, .maccatalyst, .macos, .tvos, .watchos => |os| if (is_clang) {
            ...
            // Sig provides the correct value in both places, so it
        ...
        if (ext != .ll and ext != .bc) {
            ...
            if (comp.config.link_libc) {
                if (target.isGnuLibC()) {
                    ...
                    // goal of moving more libc code to Sig, and they're also just kind of unnecessary since LLVM is
            ...
            if (comp.config.link_libcpp) {
                ...
                try argv.append(try fs.path.join(arena, &[_][]const u8{
                    comp.dirs.sig_lib.path.?, "libcxx", "include",
                }));

                try argv.append("-isystem");
                try argv.append(try fs.path.join(arena, &[_][]const u8{
                    comp.dirs.sig_lib.path.?, "libcxxabi", "include",
                }));
            ...
            try argv.append("-isystem");
            try argv.append(try fs.path.join(arena, &.{ comp.dirs.sig_lib.path.?, "include" }));
            ...
            if (comp.config.link_libunwind) {
                try argv.append("-isystem");
                try argv.append(try fs.path.join(arena, &[_][]const u8{
                    comp.dirs.sig_lib.path.?, "libunwind", "include",
                }));
            }
        ...
        => {
            ...
            switch (mod.optimize_mode) {
                ...
                .fast => {
                    ...
                    // -O3 in Sig code, the justification for the difference here is that Sig
                    // has better detection and prevention of undefined behavior, so -O3 is safer for
                    // Sig code than it is for C code. Also, C programmers are used to their code
...
pub fn addCCArgs(
    ...
    const llvm_triple = try std.sig.llvm.Builder.tripleForTarget(arena, target);
        ...
        => {
            ...
            for (all_features_list, 0..) |feature, index_usize| {
                ...
                if (feature.llvm_name) |llvm_name| {
                    ...
                    // See https://github.com/ziglang/Sig/issues/23539
...
pub const FileExt = enum {
    ...
    /// The value accepted by "Sig clang -x <lang>" and passed to "clang -x <lang>".
    ...
    /// Supported languages for "Sig clang -x <lang>".
...
test "classifyFileExt" {
    ...
    try std.testing.expectEqual(FileExt.sig, classifyFileExt("foo.sig"));
    try std.testing.expectEqual(FileExt.sig, classifyFileExt("foo.sig"));
}
...
pub fn build_crt_file(
    ...
    const basename = try std.sig.binNameAlloc(gpa, .{
        .root_name = root_name,
        .cpu_arch = target.cpu.arch,
        .os_tag = target.os.tag,
        .ofmt = target.ofmt,
...
pub fn addLinkLib(comp: *Compilation, lib_name: []const u8) !void {
    ...
    // then when we create a sub-Compilation for Sig libc, it also tries to
    // build kernel32.lib.
...
/// This decides the optimization mode for all Sig-provided libraries, including
...
/// This decides whether to strip debug info for all Sig-provided libraries, including
/// compiler-rt, libcxx, libc, libunwind, etc.
