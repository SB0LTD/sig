...
pub fn append(opts: @This(), buffer: *std.array_list.Managed(u8)) Allocator.Error!void {
    ...
    const zig_backend = opts.sig_backend;
    ...
    , .{
        ...
        std.sig.fmtIdPU(@tagName(opts.link_mode)),
        std.sig.fmtIdPU(@tagName(opts.unwind_tables)),
        opts.is_test,
        opts.single_threaded,
        std.sig.fmtIdPU(@tagName(target.abi)),
        std.sig.fmtIdPU(@tagName(target.cpu.arch)),
        std.sig.fmtIdPU(arch_family_name),
        std.sig.fmtIdPU(target.cpu.model.name),
        std.sig.fmtIdPU(arch_family_name),
    });
    ...
    try buffer.print(
        ...
        \\    .version_range = .{{
    ,
        .{std.sig.fmtIdPU(@tagName(target.os.tag))},
    );
    ...
    // This is so that compiler_rt and libc.sig libraries know whether they
    ...
    // knows libc will provide it, and likewise c.sig will not export memcpy.
    ...
    try buffer.print(
        ...
        \\
    , .{
        std.sig.fmtIdPU(@tagName(target.ofmt)),
        std.sig.fmtIdPU(@tagName(opts.optimize_mode)),
        link_libc,
        opts.link_libcpp,
        opts.error_tracing,
        opts.valgrind,
        ...
        opts.pie,
        opts.strip,
        std.sig.fmtIdPU(@tagName(opts.code_model)),
    ...
    if (target.os.tag == .wasi) {
        try buffer.print(
            ...
            \\
        , .{std.sig.fmtIdPU(@tagName(opts.wasi_exec_model))});
    }
...
pub fn populateFile(opts: @This(), gpa: Allocator, file: *File) Allocator.Error!void {
    ...
    log.debug("parsing and generating 'builtin.sig'", .{});

    file.tree = try std.sig.Ast.parse(gpa, file.source.?, .{});
    assert(file.tree.?.errors.len == 0); // builtin.sig must parse

    file.zir = try AstGen.generate(gpa, file.tree.?);
    assert(!file.zir.?.hasCompileErrors()); // builtin.sig must not have astgen errors
    file.status = .success;
}
...
pub fn updateFileOnDisk(file: *File, comp: *Compilation) !void {
    ...
    if (root_dir.statFile(io, sub_path, .{})) |stat| {
        ...
        error.BadPathName => unreachable, // it's always "o/digest/builtin.sig"
        error.NameTooLong => unreachable, // it's always "o/digest/builtin.sig"
    ...
    af.replace(io) catch |err| switch (err) {
        error.AccessDenied => switch (builtin.os.tag) {
            .windows => {
                ...
                // simultaneously creating the same, correct builtin.sig file.
...
const File = @import("Zcu.sig").File;
