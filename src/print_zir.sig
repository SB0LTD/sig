...
const Writer = struct {
    ...
    /// Using `std.sig.findLineColumn` whenever we need to resolve a source location makes ZIR
    ...
    line_col_cursor: struct {
        ...
        fn find(cur: *@This(), source: []const u8, want_offset: usize) std.sig.Loc {
            ...
            // Seek forward as in `std.sig.findLineColumn`.
    ...
    fn writeStr(
        ...
        try stream.print("\"{f}\")", .{std.sig.fmtString(str)});
    }
    ...
    fn writeParam(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        ...
        try stream.print("\"{f}\", ", .{
            std.sig.fmtString(self.code.nullTerminatedString(extra.data.name)),
        });
    ...
    fn writeAsm(
        ...
        if (tmpl_is_expr) {
            ...
            try stream.print("\"{f}\"", .{std.sig.fmtString(asm_source)});
        }
        try stream.writeAll(", ");
        ...
        var output_type_bits = extra.data.output_type_bits;
        {
            ...
            while (i < small.outputs_len) : (i += 1) {
                ...
                try stream.print("output({f}, \"{f}\", ", .{
                    std.sig.fmtIdP(name), std.sig.fmtString(constraint),
                });
    ...
    fn writeCall(
        ...
        switch (kind) {
            ...
            .field => {
                ...
                try stream.print(", \"{f}\"", .{std.sig.fmtString(field_name)});
            },
        }
        try stream.writeAll(", [");
    ...
    fn writeStructDecl(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        ...
        if (struct_decl.field_names.len == 0) {
            ...
            while (it.next()) |field| {
                ...
                try stream.print("{f}: ", .{std.sig.fmtIdP(field_name)});
            ...
            for (case.item_infos, 0..) |item_info, i| {
                ...
                switch (item_info.unwrap()) {
                    ...
                    .error_value => |str_index| {
                        const str = self.code.nullTerminatedString(str_index);
                        try stream.print("\"error.{f}\"", .{std.sig.fmtString(str)});
                    },
    ...
    fn writePlNodeField(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        ...
        try self.writeInstRef(stream, extra.lhs);
        try stream.print(", \"{f}\") ", .{std.sig.fmtString(name)});
        try self.writeSrcNode(stream, inst_data.src_node);
    }
    ...
    fn writeCapture(self: *Writer, stream: *std.Io.Writer, capture: Zir.Inst.Capture) !void {
        switch (capture.unwrap()) {
            ...
            .decl_val => |str| try stream.print("decl_val \"{f}\"", .{
                std.sig.fmtString(self.code.nullTerminatedString(str)),
            }),
    ...
    fn writeImport(self: *Writer, stream: *std.Io.Writer, inst: Zir.Inst.Index) !void {
        ...
        try stream.print(", \"{f}\") ", .{std.sig.fmtString(import_path)});
        try self.writeSrcTok(stream, inst_data.src_tok);
    }
};
