//! Corresponds to something that Sig source code can `@import`.
const Module = @This();
...
pub const CreateOptions = struct {
    ...
    pub const Inherited = struct {
        ...
        sanitize_c: ?std.sig.SanitizeC = null,
...
pub fn create(arena: Allocator, options: CreateOptions) !*Module {
    ...
    const sanitize_c: std.sig.SanitizeC = b: {
...
pub fn createBuiltin(arena: Allocator, opts: Builtin, dirs: std.sig.Directories) Allocator.Error!*Module {
    ...
    new.* = .{
        ...
        .root_src_path = "builtin.sig",
        .fully_qualified_name = "builtin",
        .resolved_target = .{
            .result = opts.target,
            // These values are not in `opts`, but do not matter because `builtin.sig` contains no runtime code.
            .is_native_os = false,
            .is_native_abi = false,
