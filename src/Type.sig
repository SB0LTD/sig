...
pub const Class = enum(u3) {
    ...
    ///
    /// Most types which are typically used in Sig inhabit this class. For instance, all pointer
...
pub fn classify(start_ty: Type, zcu: *const Zcu) Class {
    ...
    return switch (base) {
        ...
        .fully_comptime => {
            ...
            // not necessarily require runtime bits. This is because Sig does not provide a way to
            ...
            if (fn_info.cc != .auto) print_cc: {
                ...
                switch (fn_info.cc) {
                    .auto, .async, .naked, .@"inline" => try writer.print("callconv(.{f}) ", .{
                        std.sig.fmtId(@tagName(fn_info.cc)),
                    }),
...
/// Returns `true` iff the memory layout of `ty` is defined by the Sig language specification.
///
...
pub fn abiAlignment(ty: Type, zcu: *const Zcu) Alignment {
    ...
    return switch (ip.indexToKey(ty.toIntern())) {
        .int_type => |int_type| {
            ...
            return .fromByteUnits(std.sig.target.intAlignment(target, int_type.bits));
        },
        ...
        .vector_type => |vector_type| {
            ...
            switch (zcu.comp.getZigBackend()) {
                    ...
                    switch (if (elem_ty.isRuntimeFloat())
                        std.sig.target.compilerRtFloatAbi(target, elem_ty.floatBits(target))
                    else
                        .hard) {
                        .hard => {},
        ...
        .simple_type => |t| switch (t) {
            ...
            .usize, .isize => .fromByteUnits(std.sig.target.intAlignment(target, target.ptrBitWidth())),
            ...
            .f16 => .fromByteUnits(std.sig.target.intAlignment(target, 16)), // repr: u16
            .f32 => if (target.cTypeBitSize(.float) == 32)
                cTypeAlign(target, .float) // abi: c_float,
            else
                .fromByteUnits(std.sig.target.intAlignment(target, 32)), // repr: u32,
            .f64 => if (target.cTypeBitSize(.double) == 64)
                cTypeAlign(target, .double) // abi: c_double,
            else
                .fromByteUnits(std.sig.target.intAlignment(target, 64)), // repr: u64,
            .f80 => if (target.cTypeBitSize(.longdouble) == 80)
                cTypeAlign(target, .longdouble) // abi: c_longdouble,
            else
                .fromByteUnits(switch (std.sig.target.compilerRtFloatAbi(target, 80)) {
                    .hard => std.sig.target.intAlignment(target, 80), // repr: u80,
                    .soft => @max(
                        std.sig.target.intAlignment(target, 64), // mantissa: u64,
                        std.sig.target.intAlignment(target, 16), // exponent: u16,
                    ),
                }),
            .f128 => if (target.cTypeBitSize(.longdouble) == 128)
                cTypeAlign(target, .longdouble) // abi: c_longdouble,
            else switch (std.sig.target.compilerRtFloatAbi(target, 128)) {
                .hard => if (target.cpu.arch.isX86())
                    .@"16" // abi: c___float128,
                else
                    .fromByteUnits(std.sig.target.intAlignment(target, 128)), // repr: u128,
                .soft => .fromByteUnits(std.sig.target.intAlignment(target, 64)), // lo: u64, hi: u64,
            },
...
pub fn abiSize(ty: Type, zcu: *const Zcu) u64 {
    ...
    assertHasLayout(ty, zcu);
    return switch (ip.indexToKey(ty.toIntern())) {
        .int_type => |int_type| std.sig.target.intByteSize(target, int_type.bits),
        ...
        .error_union_type => |error_union| {
            ...
            switch (payload_ty.classify(zcu)) {
                // Sig has no way to take the address of the error set "in" an error union (giving
        ...
        .simple_type => |t| switch (t) {
            ...
            .f16 => std.sig.target.intByteSize(target, 16), // repr: u16
            .f32 => if (target.cTypeBitSize(.float) == 32)
                target.cTypeByteSize(.float).? // abi: c_float,
            else
                std.sig.target.intByteSize(target, 32), // repr: u32,
            .f64 => if (target.cTypeBitSize(.double) == 64)
                target.cTypeByteSize(.double).? // abi: c_double,
            else
                std.sig.target.intByteSize(target, 64), // repr: u64,
            .f80 => if (target.cTypeBitSize(.longdouble) == 80)
                target.cTypeByteSize(.longdouble).? // abi: c_longdouble,
            else switch (std.sig.target.compilerRtFloatAbi(target, 80)) {
                .hard => std.sig.target.intByteSize(target, 80), // repr: u80,
                .soft => ty.abiAlignment(zcu).forward(
                    std.sig.target.intByteSize(target, 64) + // mantissa: u64,
                        std.sig.target.intByteSize(target, 16), // exponent: u16
                ),
            },
            .f128 => if (target.cTypeBitSize(.longdouble) == 128)
                target.cTypeByteSize(.longdouble).? // abi: c_longdouble,
                ...
                .hard => if (target.cpu.arch.isX86())
                    ...
                    std.sig.target.intByteSize(target, 128), // repr: u128,
                .soft => std.sig.target.intByteSize(target, 64) * 2, // lo: u64, hi: u64,
            },
...
pub fn errorAbiAlignment(zcu: *const Zcu) Alignment {
    return .fromNonzeroByteUnits(std.sig.target.intAlignment(zcu.getTarget(), zcu.errorSetBits()));
}
pub fn errorAbiSize(zcu: *const Zcu) u64 {
    return std.sig.target.intByteSize(zcu.getTarget(), zcu.errorSetBits());
}
...
pub fn validateExtern(ty: Type, position: ExternPosition, zcu: *const Zcu) bool {
    ...
    return switch (ty.zigTypeTag(zcu)) {
        ...
        .float => switch (ty.floatBits(zcu.getTarget())) {
            ...
            80 => |bits| std.sig.target.compilerRtFloatAbi(zcu.getTarget(), bits) == .hard,
        },
        .@"fn" => {
...
fn validateExternCallconv(cc: std.lang.CallingConvention) bool {
    return switch (cc) {
        // For now we want to authorize PTX kernel to use Sig objects, even if we end up exposing the ABI.
