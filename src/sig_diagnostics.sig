...
test "detect direct allocation" {
    ...
    const entries = try analyzeSource(gpa, source, "test.sig", .default);
...
test "format diagnostic default mode" {
    ...
    try std.testing.expectEqualStrings("src/main.sig:10:5: warning: direct allocation in 'init'", msg);
}
...
test "format diagnostic strict mode" {
    ...
    try std.testing.expectEqualStrings("src/main.sig:10:5: error: direct allocation in 'init'", msg);
}
...
test "no false positives on clean code" {
    ...
    ;
    const gpa = std.testing.allocator;
    const entries = try analyzeSource(gpa, source, "clean.sig", .default);
    defer freeEntries(gpa, entries);
