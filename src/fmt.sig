...
const usage_fmt =
    ...
    \\  --ast-check            Run Sig ast-check on every file
...
pub fn run(gpa: Allocator, arena: Allocator, io: Io, args: []const []const u8) !void {
    ...
    if (stdin_flag) {
        ...
        const source_code = std.sig.readSourceFileToEndAlloc(gpa, &file_reader) catch |err| {
        ...
        if (check_ast_flag) {
            if (!force_zon) {
                ...
                const zoir = try std.sig.ZonGen.generate(gpa, tree, .{});
            ...
            std.sig.printAstErrorsToStderr(gpa, io, tree, "<stdin>", color) catch {};
            process.exit(2);
        }
...
fn fmtPathDir(
    ...
    while (try dir_it.next(io)) |entry| {
        ...
        if (is_dir or entry.kind == .file and (mem.endsWith(u8, entry.name, ".sig") or mem.endsWith(u8, entry.name, ".zon"))) {
...
fn fmtPathFile(
    ...
    const gpa = fmt.gpa;
    const source_code = std.sig.readSourceFileToEndAlloc(gpa, &file_reader) catch |err| switch (err) {
    ...
    if (tree.errors.len != 0) {
        try std.sig.printAstErrorsToStderr(gpa, io, tree, file_path, fmt.color);
        fmt.any_error = true;
        return;
    }

    if (fmt.check_ast) {
        if (stat.size > std.sig.max_src_size)
        ...
        switch (mode) {
            ...
            .zon => {
                var zoir = try std.sig.ZonGen.generate(gpa, tree, .{});
                defer zoir.deinit(gpa);
