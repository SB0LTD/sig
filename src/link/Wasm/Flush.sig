...
pub fn finish(f: *Flush, wasm: *Wasm) !void {
    ...
    if (f.sorted_init_funcs.items.len > 0) {
        // Sig has no constructors so these are only for object file inputs.
...
fn emitProducerSection(gpa: Allocator, binary_bytes: *ArrayList(u8)) !void {
    ...
    // language field
    {
        ...
        // versioned name
        {
            try appendLeb128(gpa, binary_bytes, @as(u32, 3)); // len of "sig"
            try binary_bytes.appendSlice(gpa, "sig");
