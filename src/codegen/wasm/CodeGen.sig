...
const assembly = @import("assembly.sig");
...
/// Lowers a Sig type and its value based on a given calling convention to ensure
/// it matches the ABI.
...
fn allocInt(cg: *CodeGen, int_ty: IntType) !WValue {
    const abi_size = std.math.cast(u32, std.sig.target.intByteSize(cg.target, int_ty.bits)) orelse {
        return cg.fail("Integer ABI size exceeds max stack size", .{});
    };
    const abi_align: Alignment = .fromByteUnits(std.sig.target.intAlignment(cg.target, int_ty.bits));
...
fn emitMemoryCopy(cg: *CodeGen, dst: WValue, src: WValue, len: WValue) !void {
    ...
    if (emit_check) {
        ...
        // `dst + len` are out of memory bounds. This can easily happen in Sig in a case such
        // as:
        //
...
fn intBackingBits(cg: *CodeGen, bits: u16) u16 {
    return switch (bits) {
        ...
        1...32 => 32,
        33...64 => 64,
        else => std.sig.target.intByteSize(cg.target, bits) * 8,
    };
}
...
// wasm fN_max NaN semantics differ with Sig
...
// wasm fN_min NaN semantics differ with Sig
...
fn memset(cg: *CodeGen, elem_ty: Type, ptr: WValue, len: WValue, value: WValue) InnerError!void {
    ...
    if (cg.target.cpu.has(.wasm, .bulk_memory) and abi_size == 1) {
        ...
        if (!len0_ok) {
            ...
            // out of memory bounds. This can easily happen in Sig in a case such as:
            //
