...
pub fn defineBigInt(big: CType.BigInt, w: *Writer, zcu: *const Zcu) Writer.Error!void {
    ...
    try writeStaticAssertCTypeLayout(
        name_cty,
        std.sig.target.intByteSize(target, bits),
        .fromByteUnits(std.sig.target.intAlignment(target, bits)),
        w,
        zcu,
    );
}
...
/// If the Sig type `ty` lowers to a `struct` or `union` type, renders a forward declaration of that
...
/// If the Sig type `ty` lowers to a `typedef`, renders a typedef of that type to `void`, because
...
/// If the Sig type `ty` lowers to a `struct` or `union` type, or to a `typedef`, renders the
...
/// Emits static assertions that the size and alignment of `cty` match those of the Sig type `ty`.
...
const CType = @import("../type.sig").CType;
const Alignment = @import("../../../InternPool.sig").Alignment;

const fmtIdentSolo = @import("../../c.sig").fmtIdentSolo;
