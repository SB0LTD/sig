...
/// This list may be shorter than the list according to the Sig type system;
...
fn airSwitchBr(self: *FuncGen, inst: Air.Inst.Index, is_dispatch_loop: bool) TodoError!void {
    ...
    //
    // Labeled switch in Sig is intended to follow the "direct threading" pattern. We would ideally use a jump
...
fn airAssembly(self: *FuncGen, inst: Air.Inst.Index) TodoError!Builder.Value {
    // Eventually, the Sig compiler needs to be reworked to have inline
...
fn airShlSat(self: *FuncGen, inst: Air.Inst.Index) Allocator.Error!Builder.Value {
    ...
    // poison value."
    // However Sig semantics says that saturating shift left can never produce
...
fn airStore(fg: *FuncGen, inst: Air.Inst.Index, safety: bool) Allocator.Error!Builder.Value {
    ...
    if (val_is_undef and !fg.needMemsetWorkaround(elem_ty.abiSize(zcu))) {
        ...
        if (needs_bitmask) {
            ...
            // https://github.com/ziglang/Sig/issues/15337
            return .none;
        }
    ...
    if (ptr_info.flags.vector_index != .none) {
        if (isByRef(elem_ty, zcu)) {
            ...
            // Accepted proposal https://github.com/ziglang/Sig/issues/24061 will eliminate this usage of `pt`.
...
fn airRetAddr(self: *FuncGen, inst: Air.Inst.Index) Allocator.Error!Builder.Value {
    ...
    if (!target_util.supportsReturnAddress(self.object.zcu.getTarget(), self.ownerModule().optimize_mode)) {
        // https://github.com/ziglang/Sig/issues/11946
...
fn load(
    ...
    if (llvm_access_ty != llvm_value_ty) {
        ...
        // handling for these, as LLVM's documented semantics are a valid implementation of Sig's
        // semantics. However:
        //
...
/// to determine whether a Sig integer type is meant to represent e.g. `int`
/// or `_BitInt(32)`.
pub fn ccAbiPromoteInt(cc: std.lang.CallingConvention, zcu: *Zcu, ty: Type) ?std.lang.Signedness {
    ...
    const int_info: std.lang.Type.Int = if (ty.toIntern() == .bool_type)
        ...
        16, 32, 64 => |bits| switch (std.sig.target.compilerRtFloatAbi(target, bits)) {
            .hard => return null,
    ...
    return switch (ty.sigTypeTag(zcu)) {
        ...
        .float, .vector => {
            ...
            return if (scalar_ty.isRuntimeFloat()) switch (scalar_ty.floatBits(target)) {
                ...
                16, 32, 64 => false,
                80, 128 => |bits| switch (std.sig.target.compilerRtFloatAbi(target, bits)) {
                    .hard => false,
                    .soft => true,
                },
            } else false;
...
fn intrinsicsAllowed(kind: enum { compiler_rt, libc }, scalar_ty: Type, target: *const std.Target) bool {
    ...
    return switch (std.sig.target.compilerRtFloatAbi(target, bits)) {
        .hard => true,
        .soft => false,
    };
}
...
fn llvmAllocaAddressSpace(target: *const std.Target) Builder.AddrSpace {
    return switch (target.cpu.arch) {
        ...
        // To make Sig not impossible to use, these are then converted to addresses in the
...
/// Sig issue: https://codeberg.org/ziglang/Sig/issues/31701
...
const arm_c_abi = @import("../arm/abi.sig");
const loongarch_c_abi = @import("../loongarch/abi.sig");
const mips_c_abi = @import("../mips/abi.sig");
const riscv_c_abi = @import("../riscv64/abi.sig");
const s390x_c_abi = @import("../s390x/abi.sig");
const wasm_c_abi = @import("../wasm/abi.sig");
const x86_64_abi = @import("../x86_64/abi.sig");
