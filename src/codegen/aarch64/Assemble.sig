...
pub fn nextInstruction(as: *Assemble) !?Instruction {
    ...
    log.debug(
        ...
        \\=========================
        \\
    , .{std.sig.fmtString(std.mem.span(original_source))});
    ...
    for (instructions, &mut_matchers) |instruction, *matcher| matcher.* = struct {
        fn match(as: *Assemble) !?Instruction {
            ...
            inline while (true) {
                ...
                log.debug("\"{f}\" -> \"{f}\"", .{
                    std.sig.fmtString(pattern_token),
                    std.sig.fmtString(source_token),
                });
...
fn nextToken(as: *Assemble, buf: *[token_buf_len]u8, comptime opts: struct {
    ...
    while (true) c: switch (as.source[0]) {
        ...
        '<' => if (opts.placeholders) {
            ...
            if (!@inComptime()) log.debug("invalid token \"{f}\"", .{std.sig.fmtString(std.mem.span(as.source))});
            return error.InvalidSyntax;
        },
    };
}
        ...
        switch (spec) {
            .reg_alias => |reg_alias_spec| {
                const reg = aarch64.encoding.Register.parse(token) orelse {
                    log.debug("invalid register: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
                if (reg.format != .alias or reg.format.alias != reg_alias_spec) {
                    log.debug("invalid register size: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                }
                return reg.alias;
            },
            .reg => |reg_spec| {
                ...
                if (reg.alias == if (reg_spec.allow_sp) .zr else .sp) {
                    log.debug("invalid register usage: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                }
                return reg;
            },
            .arrangement => |arrangement_spec| {
                ...
                const arrangement = std.meta.stringToEnum(Result, std.ascii.lowerString(
                    ...
                    log.debug("invalid arrangement: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
            ...
            .systemreg => {
                const systemreg = Result.parse(token) orelse {
                    log.debug("invalid system register: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
            ...
            .imm => |imm_spec| {
                ...
                if (imm_spec.multiple_of) |multiple_of| if (@rem(imm, multiple_of) != 0) {
                    log.debug("invalid immediate usage: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
                if (imm_spec.min_valid) |min_valid| if (imm < min_valid) {
                    log.debug("out of range immediate: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
            ...
            .shift => |shift_spec| {
                ...
                const shift = std.meta.stringToEnum(Result, std.ascii.lowerString(
                    ...
                    log.debug("invalid shift: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
                if (!shift_spec.allow_ror and shift == .ror) {
                    log.debug("invalid shift usage: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                }
                return shift;
            },
            .barrier => |barrier_spec| {
                ...
                const barrier = std.meta.stringToEnum(Result, std.ascii.lowerString(
                    ...
                    log.debug("invalid barrier: \"{f}\"", .{std.sig.fmtString(token)});
                    return null;
                };
