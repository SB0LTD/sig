...
pub const Inst = struct {
    ...
    comptime {
        if (!std.debug.runtime_safety) {
            ...
            // Note that in safety builds, Sig is allowed to insert a secret field for safety checks.
...
pub fn emit(
    ...
    e.emitMir() catch |err| switch (err) {
        ...
        error.InvalidInstruction, error.CannotEncode => return zcu.codegenFail(nav, "emit MIR failed: {s} (Sig compiler bug)", .{@errorName(err)}),
...
pub fn emitLazy(
    ...
    e.emitMir() catch |err| switch (err) {
        ...
        error.InvalidInstruction, error.CannotEncode => return zcu.codegenFailType(lazy_sym.ty, "emit MIR failed: {s} (Sig compiler bug)", .{@errorName(err)}),
