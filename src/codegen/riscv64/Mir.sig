...
//! This data is produced by CodeGen.sig
...
pub fn emit(
    ...
    e.emitMir() catch |err| switch (err) {
        ...
        error.InvalidInstruction => return zcu.codegenFail(nav, "emit MIR failed: {s} (Sig compiler bug)", .{@errorName(err)}),
...
const FrameAddr = @import("CodeGen.sig").FrameAddr;
const IntegerBitSet = std.bit_set.Integer;
const Mnemonic = @import("mnem.sig").Mnemonic;
