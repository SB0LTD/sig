...
const mnem_import = @import("mnem.sig");
...
pub fn generate(
    ...
    function.gen() catch |err| switch (err) {
        error.OutOfRegisters => return function.fail("ran out of registers (Sig compiler bug)", .{}),
        else => |e| return e,
    };
...
pub fn generateLazy(
    ...
    emit.emitMir() catch |err| switch (err) {
        ...
        error.InvalidInstruction => |e| return function.fail("emit MIR failed: {s} (Sig compiler bug)", .{@errorName(e)}),
        else => |e| return e,
    };
}
    ...
    for (body) |inst| {
        ...
        if (std.debug.runtime_safety) {
            if (func.air_bookkeeping < old_air_bookkeeping + 1) {
                std.debug.panic("in codegen.sig, handling of AIR instruction %{d} ('{}') did not do proper bookkeeping. Look for a missing call to finishAir.", .{ inst, air_tags[@backingInt(inst)] });
            }
...
fn airTrunc(func: *Func, inst: Air.Inst.Index) !void {
    ...
    // we assume no zeroext in the "Sig ABI", so it's fine to just not truncate it.
        ...
        .indirect => |reg_off| {
            const load_tag: Mnemonic = switch (reg.class()) {
                .float => switch (abi_size) {
                    1 => unreachable, // Sig does not support 8-bit floats
