...
const print_value = @import("print_value.sig");
const Value = @This();
...
pub fn toIpString(val: Value, ty: Type, pt: Zcu.PerThread) !InternPool.NullTerminatedString {
    ...
    assert(ty.sigTypeTag(zcu) == .array);
...
pub fn fieldValue(val: Value, pt: Zcu.PerThread, index: usize) !Value {
    ...
    return switch (zcu.intern_pool.indexToKey(val.toIntern())) {
        ...
        .bitpack => |bitpack| {
            ...
            const field_bit_offset: u16 = switch (ty.sigTypeTag(zcu)) {
                .@"union" => 0,
                .@"struct" => off: {
                    var off: u16 = 0;
...
pub fn anyScalarIsUndef(val: Value, zcu: *const Zcu) bool {
    switch (zcu.intern_pool.indexToKey(val.toIntern())) {
        ...
        .aggregate => |agg| {
            assert(Type.fromInterned(agg.ty).sigTypeTag(zcu) == .vector);
...
pub fn sqrt(val: Value, float_type: Type, arena: Allocator, pt: Zcu.PerThread) !Value {
    if (float_type.sigTypeTag(pt.zcu) == .vector) {
...
pub fn sin(val: Value, float_type: Type, arena: Allocator, pt: Zcu.PerThread) !Value {
    const zcu = pt.zcu;
    if (float_type.sigTypeTag(zcu) == .vector) {
...
pub fn ptrOptPayload(parent_ptr: Value, pt: Zcu.PerThread) !Value {
    ...
    assert(opt_ty.sigTypeTag(zcu) == .optional);
...
pub fn ptrEuPayload(parent_ptr: Value, pt: Zcu.PerThread) !Value {
    ...
    assert(eu_ty.sigTypeTag(zcu) == .error_union);
...
pub fn ptrField(parent_ptr: Value, field_idx: u32, pt: Zcu.PerThread) !Value {
    ...
    switch (aggregate_ty.sigTypeTag(zcu)) {
...
pub fn pointerDerivation(ptr_val: Value, arena: Allocator, pt: Zcu.PerThread, opt_sema: ?*Sema) Allocator.Error!PointerDeriveStep {
    ...
    if (need_child.comptimeOnly(zcu) or
        need_child.sigTypeTag(zcu) == .@"opaque" or
        ...
        switch (cur_ty.sigTypeTag(zcu)) {
            ...
            .optional => {
                ptr_opt: {
                    ...
                    if (need_child.sigTypeTag(zcu) != .pointer) break :ptr_opt;
...
pub fn interpret(val: Value, comptime T: type, pt: Zcu.PerThread) error{ OutOfMemory, UndefinedValue, TypeMismatch }!T {
    ...
    const ip = &zcu.intern_pool;
    const ty = val.typeOf(zcu);
    if (ty.sigTypeTag(zcu) != @typeInfo(T)) return error.TypeMismatch;
