...
const PrintZon = struct {
    ...
    fn renderNode(pz: *PrintZon, node: Zoir.Node.Index) Error!void {
        ...
        switch (node.get(zoir)) {
            ...
            .enum_literal => |x| try pz.w.print("enum_literal({f})", .{std.sig.fmtIdP(x.get(zoir))}),
            .string_literal => |x| try pz.w.print("str(\"{f}\")", .{std.sig.fmtString(x)}),
            ...
            .struct_literal => |s| {
                ...
                for (s.names, 0..s.vals.len) |name, idx| {
                    try pz.newline();
                    try pz.w.print("[{f}] ", .{std.sig.fmtIdP(name.get(zoir))});
