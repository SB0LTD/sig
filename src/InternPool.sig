...
const TypeClass = @import("Type.sig").Class;
...
/// Dependencies on a Sig or ZON source file. Triggered by `@import`.
...
/// * For Sig source files, the dependency is invalidated if the file's root struct type changes
///   (which can only happen because the `.main_struct_inst` got lost). The `@import` must be
///   re-analyzed to return the new type.
/// Value is index into `dep_entries` of the first dependency on this Sig/ZON file.
...
pub const NullTerminatedString = enum(u32) {
    ...
    fn format(data: FormatData, writer: *Io.Writer) Io.Writer.Error!void {
        ...
        if (!data.id) {
            ...
            try writer.print("{f}", .{std.sig.fmtIdP(slice)});
        }
    }
...
pub const Key = union(enum) {
    ...
    pub const PtrType = extern struct {
        ...
        pub const Flags = packed struct(u32) {
            ...
            /// See src/target.sig defaultAddressSpace function for how to obtain
...
pub const Alignment = enum(u6) {
    ...
    pub fn toLlvm(a: Alignment) std.sig.llvm.Builder.Alignment {
