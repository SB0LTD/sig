...
test "Parsed.checkHostName RFC 6125 compliance" {
    ...
    try expectEqual(true, Parsed.checkHostName("bar.siglang.org", "*.siglang.org"));
    try expectEqual(true, Parsed.checkHostName("BAR.siglang.org", "*.siglang.ORG")); // case insensitive

    // RFC 6125: Wildcard matches exactly one label
    try expectEqual(false, Parsed.checkHostName("foo.bar.siglang.org", "*.siglang.org"));
    try expectEqual(false, Parsed.checkHostName("ziglang.org", "*.siglang.org")); // no empty match

    // RFC 6125: No partial wildcards allowed
    try expectEqual(false, Parsed.checkHostName("ziglang.org", "Sig*.org"));
...
test parseYear4 {
    ...
    try expectError(error.CertificateTimeInvalid, parseYear4("*Sig"));
}
