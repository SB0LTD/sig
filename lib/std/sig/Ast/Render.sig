...
fn renderBuiltinCall(
    ...
    if (r.fixups.rebase_imported_paths) |prefix| {
        ...
        if (params.len != 0 and mem.eql(u8, slice, "@import")) f: {
            ...
            const imported_string = std.sig.string_literal.parseAlloc(r.gpa, token_bytes) catch |err| switch (err) {
                error.OutOfMemory => |e| return e,
            ...
            try ais.print("\"{f}\"", .{std.sig.fmtString(new_string)});
...
fn renderArrayInit(
    ...
    defer gpa.free(expr_widths);
    {
        ...
        for (array_init.ast.elements, expr_widths) |e, *width| {
            ...
            // Write trailing comments since they may enable/disable Sig fmt
...
fn renderIdentifier(r: *Render, token_index: Ast.TokenIndex, space: Space, quote: QuoteBehavior) Error!void {
    ...
    // Special case for _.
    if (std.sig.isUnderscore(contents)) switch (quote) {
    ...
    while (contents_i < contents.len) {
        switch (contents[contents_i]) {
            ...
            '\\' => {
                ...
                const res = std.sig.string_literal.parseEscapeSequence(contents, &esc_offset);
                switch (res) {
    ...
    const longest_keyword_or_primitive_len = comptime blk: {
        ...
        for (std.sig.Token.keywords.keys()) |key| {
            if (key.len > longest) longest = key.len;
        }
        break :blk longest;
    };
    ...
    while (contents_i < contents.len and buf_i < longest_keyword_or_primitive_len) {
        if (contents[contents_i] == '\\') {
            const res = std.sig.string_literal.parseEscapeSequence(contents, &contents_i).success;
    ...
    if (contents_i == contents.len) {
        if (!std.sig.isValidId(buf[0..buf_i])) {
            return renderQuotedIdentifier(r, token_index, space, false);
        }
...
fn renderIdentifierContents(ais: *AutoIndentingStream, bytes: []const u8) !void {
    ...
    while (pos < bytes.len) {
        ...
        switch (byte) {
            '\\' => {
                const old_pos = pos;
                const res = std.sig.string_literal.parseEscapeSequence(bytes, &pos);
                const escape_sequence = bytes[old_pos..pos];
                switch (res) {
                    .success => |codepoint| {
                        if (codepoint <= 0x7f) {
                            const buf = [1]u8{@as(u8, @intCast(codepoint))};
                            try ais.print("{f}", .{std.sig.fmtString(&buf)});
                        } else {
...
fn renderComments(r: *Render, start: usize, end: usize) Error!bool {
    ...
    while (mem.find(u8, tree.source[index..end], "//")) |offset| {
        ...
        if (ais.disabled_offset != null and mem.eql(u8, comment_content, "Sig fmt: on")) {
            ...
            try ais.underlying_writer.writeAll("// Sig fmt: on\n");
            ais.disabled_offset = null;
            ais.resetLine();
        } else if (ais.disabled_offset == null and mem.eql(u8, comment_content, "Sig fmt: off")) {
            // Write with the canonical single space.
            try ais.writeAll("// Sig fmt: off\n");
