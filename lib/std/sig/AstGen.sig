...
const isPrimitive = std.sig.primitives.isPrimitive;

const Zir = std.sig.Zir;
const BuiltinFn = std.sig.BuiltinFn;
const AstRlAnnotate = std.sig.AstRlAnnotate;
...
fn reachableExprComptime(
    ...
    comptime_reason: ?std.sig.SimpleComptimeReason,
) InnerError!Zir.Inst.Ref {
...
/// Turn Sig AST into untyped ZIR instructions.
...
fn expr(gz: *GenZir, scope: *Scope, ri: ResultInfo, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    switch (tree.nodeTag(node)) {
        ...
        // TODO restore this when implementing https://github.com/ziglang/Sig/issues/6025
...
fn comptimeExpr(
    ...
    ri: ResultInfo,
    node: Ast.Node.Index,
    reason: std.sig.SimpleComptimeReason,
) InnerError!Zir.Inst.Ref {
...
fn fnDecl(
    ...
    astgen.src_hasher = std.sig.SrcHasher.init(.{});
    ...
    var hash: std.sig.SrcHash = undefined;
...
fn fnDeclInner(
    ...
    var proto_hash: std.sig.SrcHash = undefined;
...
fn structDeclInner(
    ...
    var fields_hash: std.sig.SrcHash = undefined;
...
fn whileExpr(
    ...
    // Tracking issue: https://github.com/ziglang/Sig/issues/9185
...
fn charLiteral(gz: *GenZir, ri: ResultInfo, node: Ast.Node.Index) InnerError!Zir.Inst.Ref {
    ...
    switch (std.sig.parseCharLiteral(slice)) {
        .success => |codepoint| {
...
fn numberLiteral(gz: *GenZir, ri: ResultInfo, node: Ast.Node.Index, source_node: Ast.Node.Index, sign: Sign) InnerError!Zir.Inst.Ref {
    ...
    const result: Zir.Inst.Ref = switch (std.sig.parseNumberLiteral(bytes)) {
        .int => |num| switch (num) {
...
fn failWithNumberError(astgen: *AstGen, err: std.sig.number_literal.Error, token: Ast.TokenIndex, bytes: []const u8) InnerError {
...
fn asmExpr(
    ...
    // See https://github.com/ziglang/Sig/issues/215 and related issues discussing
    // possible inline assembly improvements. Until then here is status quo AstGen
    // for assembly syntax. It's used by std lib crypto aesni.sig.
    ...
    switch (info.tag) {
        ...
        .import => {
            ...
            if (tree.nodeTag(operand_node) != .string_literal) {
                // Spec reference: https://github.com/ziglang/Sig/issues/2206
...
comptime {
    // These checks ensure that std.sig.primitives stays in sync with the primitive->Zir map.
    const primitives = std.sig.primitives;
    for (primitive_instrs.keys(), primitive_instrs.values()) |key, value| {
        if (!primitives.isPrimitive(key)) {
            @compileError("std.sig.isPrimitive() is not aware of Zir instr '" ++ @tagName(value) ++ "'");
        }
    }
    for (primitives.names.keys()) |key| {
        if (primitive_instrs.get(key) == null) {
            @compileError("std.sig.primitives entry '" ++ key ++ "' does not have a corresponding Zir instr");
        }
    }
}
...
fn nodeIsTriviallyZero(tree: *const Ast, node: Ast.Node.Index) bool {
    switch (tree.nodeTag(node)) {
        .number_literal => {
            ...
            return switch (std.sig.parseNumberLiteral(tree.tokenSlice(ident))) {
...
fn parseStrLit(
    ...
    const result = r: {
        ...
        break :r std.sig.string_literal.parseWrite(&aw.writer, raw_string) catch |err| switch (err) {
...
fn failWithStrLitError(
    astgen: *AstGen,
    err: std.sig.string_literal.Error,
    token: Ast.TokenIndex,
    bytes: []const u8,
    offset: u32,
) InnerError {
...
const GenZir = struct {
    ...
    fn setBlockComptimeBody(gz: *GenZir, inst: Zir.Inst.Index, comptime_reason: std.sig.SimpleComptimeReason) !void {
        const astgen = gz.astgen;
        const gpa = astgen.gpa;
        const body = gz.instructionsSlice();
        const body_len = astgen.countBodyLenAfterFixups(body);
