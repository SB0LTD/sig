...
const oracle = @import("parser_generated_oracle.sig");

test "fuzz std.sig.Ast.parse() against generated oracle" {
...
fn checkAgainstOracle(source: [:0]const u8) !void {
    ...
    // error right away and does no recovery. However, std.sig.Ast.parse() does recovery
    ...
    const ast = try std.sig.Ast.parse(fba.allocator(), source, .{ .recover = false });

    errdefer logBadSource(source, ast);
    try std.testing.expectEqual(expected, ast.errors.len == 0);
}

fn logBadSource(source: []const u8, ast: std.sig.Ast) void {
...
fn logBadSourceInner(source: []const u8, t: std.Io.Terminal, ast: std.sig.Ast) std.Io.Writer.Error!void {
    @disableInstrumentation();
