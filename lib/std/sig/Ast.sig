//! Abstract Syntax Tree for Sig source code.
//! For Sig syntax, the root node is at nodes[0] and contains the list of
//! sub-nodes.
...
pub fn parse(gpa: Allocator, source: [:0]const u8, options: ParseOptions) Allocator.Error!Ast {
    ...
    // Empirically, the Sig std lib has an 8:1 ratio of source bytes to token count.
    const estimated_token_count = source.len / 8;
    try tokens.ensureTotalCapacity(gpa, estimated_token_count);

    var tokenizer = std.sig.Tokenizer.init(source);
    while (true) {
...
pub fn parseTokens(
    ...
    // Empirically, Sig source code has a 2:1 ratio of tokens to AST nodes.
...
pub const Render = @import("Ast/Render.sig");
...
pub fn tokenSlice(tree: Ast, token_index: TokenIndex) []const u8 {
    ...
    var tokenizer: std.sig.Tokenizer = .{
        .buffer = tree.source,
    ...
    switch (parse_error.tag) {
        ...
        .invalid_byte => {
            ...
            return w.print("{s} contains invalid byte: '{f}'", .{
                ...
                std.sig.fmtChar(tok_slice[parse_error.extra.offset]),
            });
        },
