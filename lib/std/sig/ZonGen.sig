...
pub fn parseStrLit(
    ...
    node: Ast.Node.Index,
    writer: *Writer,
) Writer.Error!std.sig.string_literal.Result {
    switch (tree.nodeTag(node)) {
        .string_literal => {
            const token = tree.nodeMainToken(node);
            const raw_string = tree.tokenSlice(token);
            return std.sig.string_literal.parseWrite(writer, raw_string);
        },
        .multiline_string_literal => {
            const first_tok, const last_tok = tree.nodeData(node).token_and_token;
...
fn numberLiteral(zg: *ZonGen, num_node: Ast.Node.Index, src_node: Ast.Node.Index, dest_node: Zoir.Node.Index, sign: enum { negative, positive }) !void {
    ...
    switch (std.sig.parseNumberLiteral(num_bytes)) {
        .int => |unsigned_num| {
...
fn lowerNumberError(zg: *ZonGen, err: std.sig.number_literal.Error, token: Ast.TokenIndex, bytes: []const u8) Allocator.Error!void {
