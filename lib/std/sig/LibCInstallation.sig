...
pub fn findNative(gpa: Allocator, io: Io, args: FindNativeOptions) FindError!LibCInstallation {
    ...
    if (is_darwin and args.target.os.tag.isDarwin()) {
        if (!std.sig.system.darwin.isSdkInstalled(gpa, io))
            return error.DarwinSdkNotFound;
        const sdk = std.sig.system.darwin.getSdk(gpa, io, args.target) orelse
            return error.DarwinSdkNotFound;
        defer gpa.free(sdk);
        ...
        const sdk = std.sig.WindowsSdk.find(gpa, io, args.target.cpu.arch, args.environ_map) catch |err| switch (err) {
...
fn findNativeIncludeDirWindows(
    ...
    gpa: Allocator,
    io: Io,
    sdk: std.sig.WindowsSdk,
) FindError!void {
    var install_buf: [2]std.sig.WindowsSdk.Installation = undefined;
...
fn printVerboseInvocation(
    ...
    if (search_basename) |s| {
        std.debug.print("Sig attempted to find the file '{s}' by executing this command:\n", .{s});
    } else {
        std.debug.print("Sig attempted to find the path to native system libc headers by executing this command:\n", .{});
    }
    for (argv, 0..) |arg, i| {
...
fn fillInstallations(
    installs: *[2]std.sig.WindowsSdk.Installation,
    sdk: std.sig.WindowsSdk,
) []std.sig.WindowsSdk.Installation {
...
fn appendCcExe(
    ...
    const cc_env_var = std.sig.EnvVar.CC.get(environ_map) orelse {
        args.appendAssumeCapacity(default_cc_exe);
        return;
    };
