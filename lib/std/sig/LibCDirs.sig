...
pub fn detect(
    ...
    if (is_native_abi and !target.isMinGW()) {
        ...
        libc.* = LibCInstallation.findNative(arena, io, .{
            ...
            => |e| {
                ...
                // libc files.
                if (std.sig.target.canBuildLibC(target)) {
                    return detectFromBuilding(arena, SIG_LIB_DIR, target);
                }
                return e;
            },
            else => |e| return e,
    ...
    // If Sig can't build the libc for the target and we are targeting the
...
pub fn detectFromBuilding(arena: Allocator, SIG_LIB_DIR: Path, target: *const std.Target) !LibCDirs {
    const s = std.fs.path.sep_str;

    if (target.os.tag.isDarwin()) {
        const list = try arena.alloc([]const u8, 1);
        list[0] = try arena.print("{f}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "any-darwin-any", .{SIG_LIB_DIR});
        return .{
    ...
    const arch_name = if (target.isMuslLibC() or target.isWasiLibC())
        std.sig.target.muslArchNameHeaders(target.cpu.arch)
    else if (target.isGnuLibC())
        std.sig.target.glibcArchNameHeaders(target.cpu.arch)
    else if (target.isFreeBSDLibC())
        std.sig.target.freebsdArchNameHeaders(target.cpu.arch)
    else if (target.isNetBSDLibC())
        std.sig.target.netbsdArchNameHeaders(target.cpu.arch)
    else if (target.isOpenBSDLibC())
        std.sig.target.openbsdArchNameHeaders(target.cpu.arch)
    else
        @tagName(target.cpu.arch);
    const os_name = @tagName(target.os.tag);
    const abi_name = if (target.isMuslLibC())
        std.sig.target.muslAbiNameHeaders(target.abi)
    else if (target.isGnuLibC())
        std.sig.target.glibcAbiNameHeaders(target.abi)
    else if (target.isNetBSDLibC())
        std.sig.target.netbsdAbiNameHeaders(target.abi)
    else
        @tagName(target.abi);
    const arch_include_dir = try arena.print("{f}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "{s}-{s}-{s}", .{
        SIG_LIB_DIR, arch_name, os_name, abi_name,
    });
    const generic_include_dir = try arena.print("{f}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "generic-{s}", .{
        SIG_LIB_DIR, generic_name,
    });
    const generic_arch_name = std.sig.target.osArchName(target);
    const arch_os_include_dir = try arena.print("{f}" ++ s ++ "libc" ++ s ++ "include" ++ s ++ "{s}-{s}-any", .{
        SIG_LIB_DIR, generic_arch_name, os_name,
    });
