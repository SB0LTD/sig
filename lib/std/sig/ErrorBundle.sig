...
pub fn renderToStderr(eb: ErrorBundle, io: Io, options: RenderOptions, color: std.sig.Color) RenderToStderrError!void {
...
pub const Wip = struct {
        ...
        for (0..items_len) |_| {
            ...
            const err_span = blk: {
                if (item.data.node.unwrap()) |node| {
                    ...
                    break :blk std.sig.Ast.Span{ .start = start, .end = end, .main = start };
                } else unreachable;
            };
            const err_loc = std.sig.findLineColumn(source, err_span.main);
    ...
    pub fn addZoirErrorMessages(
        ...
        for (zoir.compile_errors) |err| {
            const err_span: std.sig.Ast.Span = span: {
            ...
            for (notes_start.., err.first_note.., 0..err.note_count) |eb_note_idx, zoir_note_idx, _| {
                ...
                const note_span: std.sig.Ast.Span = span: {
                ...
                const note_loc = std.sig.findLineColumn(source, note_span.main);
