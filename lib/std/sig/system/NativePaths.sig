...
pub fn detect(
    ...
    if (std.sig.EnvVar.NIX_CFLAGS_COMPILE.get(environ_map)) |nix_cflags_compile| {
        is_nix = true;
    ...
    if (std.sig.EnvVar.NIX_LDFLAGS.get(environ_map)) |nix_ldflags| {
        is_nix = true;
    ...
    if (std.sig.EnvVar.NIX_CFLAGS_LINK.get(environ_map)) |nix_cflags_link| {
        is_nix = true;
    ...
    if (builtin.target.os.tag.isDarwin()) {
        if (std.sig.system.darwin.isSdkInstalled(arena, io)) sdk: {
            const sdk = std.sig.system.darwin.getSdk(arena, io, native_target) orelse break :sdk;
        ...
        if (std.sig.EnvVar.HOMEBREW_PREFIX.get(environ_map)) |prefix| {
    ...
    if (builtin.os.tag != .windows and builtin.os.tag != .wasi) {
        const triple = try std.sig.target.linuxTriple(arena, native_target);
        ...
        if (std.sig.EnvVar.C_INCLUDE_PATH.get(environ_map)) |c_include_path| {
        ...
        if (std.sig.EnvVar.CPLUS_INCLUDE_PATH.get(environ_map)) |cplus_include_path| {
        ...
        if (std.sig.EnvVar.LIBRARY_PATH.get(environ_map)) |library_path| {
