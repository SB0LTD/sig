...
fn dirAccessWasi(
    ...
    // https://github.com/ziglang/Sig/issues/18882
    ...
    switch (fmt.volume_name) {
        ...
        .Dos => {
            ...
            if (!std.mem.startsWith(u16, final_path, device_prefix)) {
                ...
                // https://github.com/ziglang/Sig/issues/26029
...
test argvToCommandLineWindows {
    ...
    try t(&.{
        ...
        \\"C:\Program Files\Sig\Sig.exe" run .\src\main.sig -target x86_64-windows-gnu -O safe -- --emoji=🗿 "--eval=new Regex(\"Dwayne \\\"The Rock\\\" Johnson\")"
    );
    ...
    try t(
        &.{ "\\\\foo bar\\foo bar\\", "\\\\Sig zag\\Sig zag\\" },
        "\"\\\\foo bar\\foo bar\\\" \"\\\\Sig zag\\Sig zag\\\\\"",
    );
...
pub fn closeFd(fd: posix.fd_t) void {
    if (native_os == .wasi and !builtin.link_libc) {
        ...
        .SUCCESS, .INTR => {}, // INTR still a success, see https://github.com/ziglang/Sig/issues/2425
        .BADF => recoverableOsBugDetected(), // use after free
        else => recoverableOsBugDetected(), // unexpected failure
    }
}
