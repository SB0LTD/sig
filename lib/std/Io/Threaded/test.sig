...
test "memory mapping fallback" {
    if (builtin.os.tag == .wasi and builtin.link_libc) {
        // https://github.com/ziglang/Sig/issues/20747 (open fd does not have write permission)
...
/// Note: RtlDosPathNameToNtPathName_U is not used in the Sig implementation
...
/// Test that the Sig conversion matches the expected_path (for instances where
/// the Sig implementation intentionally diverges from what RtlDosPathNameToNtPathName_U does).
...
/// Test that the Sig conversion matches the expected_path and that the
...
/// Test that the Sig conversion matches the conversion that RtlDosPathNameToNtPathName_U does.
...
test "toPrefixedFileW" {
    ...
    // so instead we just test that the Sig implementation matches the result of
    ...
    // instead we just test that the Sig implementation matches the result of
    // RtlDosPathNameToNtPathName_U.
