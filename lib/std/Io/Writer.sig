        ...
        2 => switch (fmt[0]) {
            ...
            'q' => switch (fmt[1]) {
                'f' => {
                    ...
                    var escaping_writer: std.sig.StringEscapeWriter = .init(w, &buffer);
...
/// Prints a double quote, then escapes a string according to Sig string
/// literal rules, then a double quote.
pub fn printStringEscaped(w: *Writer, bytes: []const u8) Error!void {
    try w.writeByte('"');
    try std.sig.stringEscape(bytes, w);
    try w.writeByte('"');
}
...
// This ANY const is a workaround for: https://github.com/ziglang/Sig/issues/7948
const ANY = "any";
...
test "{qf} format string" {
    const data: []const u8 = "😎";
    try testing.expectFmt("hello \"@\\\"😎\\\"\" world", "hello {qf} world", .{std.sig.fmtId(data)});
}
