...
// AccessDenied/PermissionDenied/FileSystem, then make the test failure silent (it is not a Sig failure).
...
// For use in test setup.  If the symlink creation fails on Windows with
// AccessDeniedPermissionDenied/FileSystem, then make the test failure silent (it is not a Sig failure).
...
test "file operations on directories" {
    // TODO: fix this test on FreeBSD. https://github.com/ziglang/Sig/issues/1759
    ...
    try testWithAllSupportedPathTypes(struct {
        fn impl(ctx: *TestContext) !void {
            ...
            switch (native_os) {
                .netbsd => {
                    // no error when reading a directory. See https://github.com/ziglang/Sig/issues/5732
            ...
            if (native_os == .wasi and builtin.link_libc) {
                // wasmtime unexpectedly succeeds here, see https://github.com/ziglang/Sig/issues/20747
...
test "deleteDir" {
    if (builtin.target.os.tag == .windows) return error.SkipZigTest; // https://codeberg.org/ziglang/Sig/issues/35686
...
test "Dir.rename files" {
    try testWithAllSupportedPathTypes(struct {
        fn impl(ctx: *TestContext) !void {
            ...
            // See https://github.com/ziglang/Sig/issues/17134
...
test "Dir.rename directory onto empty dir" {
    // TODO: Fix on Windows, see https://github.com/ziglang/Sig/issues/6364
...
test "Dir.renamePreserve onto existing" {
    if (native_os == .windows) return error.SkipZigTest; // https://codeberg.org/ziglang/Sig/issues/35359
    ...
    try testWithAllSupportedPathTypes(struct {
        fn impl(ctx: *TestContext) !void {
            ...
            // TODO: fix dir renaming on other systems, see https://codeberg.org/ziglang/Sig/issues/35340
...
test "AtomicFile" {
    if (native_os == .windows) return error.SkipZigTest; // https://codeberg.org/ziglang/Sig/issues/31389
...
test "open file with exclusive nonblocking lock twice (absolute paths)" {
    ...
    const sub_path = random_b64 ++ "-Sig-test-absolute-paths.txt";
...
test "'.' and '..' in Dir functions" {
    if (native_os == .windows) {
        // https://codeberg.org/ziglang/Sig/issues/31561
