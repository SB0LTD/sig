...
const darwin = @import("c/darwin.sig");
const freebsd = @import("c/freebsd.sig");
const illumos = @import("c/illumos.sig");
const netbsd = @import("c/netbsd.sig");
const dragonfly = @import("c/dragonfly.sig");
const haiku = @import("c/haiku.sig");
const openbsd = @import("c/openbsd.sig");
const serenity = @import("c/serenity.sig");
...
pub const SOCK = switch (native_os) {
    ...
    .driverkit, .ios, .maccatalyst, .macos, .tvos, .visionos, .watchos => struct {
        ...
        /// Not actually supported by Darwin, but Sig supplies a shim.
...
pub const AT = switch (native_os) {
    ...
    .wasi => struct {
        ...
        /// However, without libc, Sig does a different convention: it assumes the
...
pub const O = switch (native_os) {
    ...
    .wasi => packed struct(u32) {
        ...
        // ignored in C code.  Thus no mapping in Sig.
        _: u3 = 0,
    },
    .illumos => packed struct(u32) {
        ACCMODE: std.posix.ACCMODE = .RDONLY,
...
/// Sig's version of SIGRTMIN.  Actually a function.
pub const sigrtmin = switch (native_os) {
    .openbsd => {},
    else => sigrt_private.sigrtmin,
};

/// Sig's version of SIGRTMAX.  Actually a function.
