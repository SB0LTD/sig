...
//!
//! For lower level control over parsing, see `std.sig.Zoir`.
...
const Base = std.sig.number_literal.Base;
const StrLitErr = std.sig.string_literal.Error;
const NumberLiteralError = std.sig.number_literal.Error;
...
pub fn fromSliceAlloc(
    ...
    var ast = try std.sig.Ast.parse(gpa, source, .{ .mode = .zon });
            ...
            inline .@"struct", .@"union", .@"enum" => |info| {
                const note: Error.TypeCheckFailure.Note = if (info.field_names.len == 0) b: {
                    ...
                    inline for (info.field_names, 0..) |field_name, i| {
                        ...
                        try buf.print(gpa, "'{f}'", .{std.sig.fmtIdFlags(field_name, .{
...
test "std.zon string literal" {
    ...
    {
        {
            var ast = try std.sig.Ast.parse(gpa, "\"abcd\"", .{ .mode = .zon });
...
test "std.zon no alloc" {
    ...
    var ast = try std.sig.Ast.parse(gpa, ".{ 1, 2, .{ 3, 4 } }", .{ .mode = .zon });
    defer ast.deinit(gpa);
