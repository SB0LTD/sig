//! Builds of the sig compiler are distributed partly in source form. That
...
const tokenizer = @import("sig/tokenizer.sig");

pub const ErrorBundle = @import("sig/ErrorBundle.sig");
pub const Server = @import("sig/Server.sig");
pub const Client = @import("sig/Client.sig");
pub const Token = tokenizer.Token;
pub const Tokenizer = tokenizer.Tokenizer;
pub const TokenSmith = @import("sig/TokenSmith.sig");
pub const string_literal = @import("sig/string_literal.sig");
pub const number_literal = @import("sig/number_literal.sig");
pub const primitives = @import("sig/primitives.sig");
pub const isPrimitive = primitives.isPrimitive;
pub const Ast = @import("sig/Ast.sig");
pub const AstGen = @import("sig/AstGen.sig");
pub const Zir = @import("sig/Zir.sig");
pub const Zoir = @import("sig/Zoir.sig");
pub const ZonGen = @import("sig/ZonGen.sig");
pub const system = @import("sig/system.sig");
pub const BuiltinFn = @import("sig/BuiltinFn.sig");
pub const AstRlAnnotate = @import("sig/AstRlAnnotate.sig");
pub const LibCInstallation = @import("sig/LibCInstallation.sig");
pub const WindowsSdk = @import("sig/WindowsSdk.sig");
pub const LibCDirs = @import("sig/LibCDirs.sig");
pub const PkgConfig = @import("sig/PkgConfig.sig");
pub const target = @import("sig/target.sig");
pub const llvm = @import("sig/llvm.sig");

pub const parser_generated_oracle = @import("sig/parser_generated_oracle.sig");
...
pub const c_translation = struct {
    pub const builtins = @import("sig/c_translation/builtins.sig");
...
/// There are many assumptions in the entire codebase that sig source files can
...
/// Returns the standard file system basename of a binary generated by the sig compiler.
...
pub const RcIncludes = enum(u2) {
    ...
    msvc,
    /// Use MinGW include paths (distributed with sig).
    gnu,
...
/// via the `-mcpu` flag passed to the sig compiler.
...
/// Return a Formatter for a sig identifier, escaping it with `@""` syntax if needed.
///
/// See also `fmtIdFlags`.
...
test fmtId {
    ...
    // These are technically not currently legal in sig.
...
pub const FormatId = struct {
    ...
    /// Print the string as a sig identifier, escaping it with `@""` syntax if needed.
...
/// Return a formatter for escaping a double quoted sig string.
pub fn fmtString(bytes: []const u8) std.fmt.Alt([]const u8, stringEscape) {
    return .{ .data = bytes };
}

/// Return a formatter for escaping a single quoted sig string.
...
/// Collects all the environment variables that sig could possibly inspect, so
/// that we can do reflection on this and print them with `sig env`.
pub const EnvVar = enum {
    SIG_GLOBAL_CACHE_DIR,
    SIG_LOCAL_CACHE_DIR,
    ZIG_LOCAL_PKG_DIR,
    SIG_LIB_DIR,
    sig_libC,
...
pub const Directories = struct {
    ...
    pub fn init(arena: Allocator, io: Io, options: InitOptions) Directories {
        ...
        const sig_lib: Cache.Directory = d: {
            if (options.override_sig_lib) |path| break :d openUnresolved(arena, io, cwd, path, .@"sig lib");
            if (wasi) break :d getPreopen(options.preopens, "/lib");
            break :d findSigLibDirFromSelfExe(arena, io, cwd, options.self_exe_path) catch |err| {
                fatal("unable to find sig installation directory from executable path {q}: {t}", .{
        ...
        const global_cache: Cache.Directory = d: {
            ...
            const path = resolveGlobalCacheDir(arena, options.environ_map) catch |err| {
                fatal("unable to resolve sig cache directory: {t}", .{err});
            };
        ...
        if (mem.eql(u8, sig_lib.path orelse "", global_cache.path orelse "")) {
            fatal("sig lib directory '{f}' cannot be equal to global cache directory '{f}'", .{ sig_lib, global_cache });
        }
        if (mem.eql(u8, sig_lib.path orelse "", local_cache.path orelse "")) {
            fatal("sig lib directory '{f}' cannot be equal to local cache directory '{f}'", .{ sig_lib, local_cache });
        }
    ...
    pub fn openUnresolved(
        ...
        thing: enum { @"sig lib", @"global cache", @"local cache", @"build root" },
    ) Cache.Directory {
        ...
        const handle_or_err = switch (thing) {
            .@"sig lib", .@"build root" => Dir.cwd().openDir(io, nonempty_path, .{}),
                ...
                const extra_str: []const u8 = e: {
                    if (thing == .@"global cache") switch (err) {
                        error.AccessDenied, error.ReadOnlyFileSystem => break :e "\n" ++
                            ...
                            "the SIG_GLOBAL_CACHE_DIR environment variable or the --global-cache-dir option.",
                        else => {},
                    };
                    break :e "";
                };
...
pub fn findSigLibDir(gpa: Allocator, io: Io) !Cache.Directory {
    const cwd_path = try getResolvedCwd(io, gpa);
    defer gpa.free(cwd_path);
    const self_exe_path = try std.process.executablePathAlloc(io, gpa);
    defer gpa.free(self_exe_path);

    return findSigLibDirFromSelfExe(gpa, io, cwd_path, self_exe_path);
}

/// Both the directory handle and the path are newly allocated resources which the caller now owns.
pub fn findSigLibDirFromSelfExe(
    allocator: Allocator,
    io: Io,
    ...
    while (Dir.path.dirname(cur_path)) |dirname| : (cur_path = dirname) {
        ...
        const sub_directory = testSigInstallPrefix(io, base_dir) orelse continue;
...
fn testSigInstallPrefix(io: Io, base_dir: Dir) ?Cache.Directory {
    const test_index_file = "std" ++ Dir.path.sep_str ++ "std.sig";

    // Sig layout: <prefix>/lib/std/std.sig (no lib/sig/ indirection)
    var test_sig_dir = base_dir.openDir(io, "lib", .{}) catch return null;
    const file = test_sig_dir.openFile(io, test_index_file, .{}) catch {
        test_sig_dir.close(io);
        return null;
    };
    file.close(io);
    return .{ .handle = test_sig_dir, .path = "lib" };
}
...
/// Searches upwards from `cwd` for a directory containing a `build.sig` file.
/// If such a directory is found, returns the path to it joined to the `.sig_cache` name.
...
pub fn resolveSuitableLocalCacheDir(arena: Allocator, io: Io, cwd: []const u8) Allocator.Error!?[]u8 {
    var cur_dir = cwd;
    while (true) {
        const joined = try Dir.path.join(arena, &.{ cur_dir, build_sig_basename });
        if (Dir.cwd().access(io, joined, .{})) |_| {
            return try Dir.path.join(arena, &.{ cur_dir, default_local_sig_cache_basename });
        } else |err| switch (err) {
            error.FileNotFound => {
...
/// Assumes `argv` has `--listen=-` in it and the child process is `sig build-exe`.
///
