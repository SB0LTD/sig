...
/// Renders the given Sig value as JSON.
///
/// Supported types:
///  * Sig `bool` -> JSON `true` or `false`.
///  * Sig `?T` -> `null` or the rendering of `T`.
///  * Sig `i32`, `u64`, etc. -> JSON number or string.
...
///  * Sig floats -> JSON number or string.
...
///  * Sig `[]const u8`, `[]u8`, `*[N]u8`, `@Vector(N, u8)`, and similar -> JSON string.
///      * See `Options.emit_strings_as_arrays`.
///      * If the content is not valid UTF-8, rendered as an array of numbers instead.
///  * Sig `[]T`, `[N]T`, `*[N]T`, `@Vector(N, T)`, and similar -> JSON array of the rendering of each item.
///  * Sig tuple -> JSON array of the rendering of each item.
///  * Sig `struct` -> JSON object with each field in declaration order.
...
///  * Sig `union(enum)` -> JSON object with one field named for the active tag and a value representing the payload.
...
///  * Sig `enum` -> JSON string naming the active tag.
...
///  * Sig untyped enum literal -> JSON string naming the active tag.
///  * Sig error -> JSON string naming the error.
///  * Sig `*T` -> the rendering of `T`. Note there is no guard against circular-reference infinite recursion.
///
        ...
        .pointer => |ptr_info| switch (ptr_info.size) {
            ...
            .many, .slice => {
                ...
                if (ptr_info.child == u8) {
                    // This is a []const u8, or some similar Sig string.
