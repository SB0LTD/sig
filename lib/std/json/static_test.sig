...
const parseFromSlice = @import("./static.sig").parseFromSlice;
const parseFromSliceLeaky = @import("./static.sig").parseFromSliceLeaky;
const parseFromTokenSource = @import("./static.sig").parseFromTokenSource;
const parseFromTokenSourceLeaky = @import("./static.sig").parseFromTokenSourceLeaky;
const innerParse = @import("./static.sig").innerParse;
const parseFromValue = @import("./static.sig").parseFromValue;
const parseFromValueLeaky = @import("./static.sig").parseFromValueLeaky;
const ParseOptions = @import("./static.sig").ParseOptions;

const Scanner = @import("Scanner.sig");

const Value = @import("./dynamic.sig").Value;
...
test "test all types" {
    if (true) return error.SkipZigTest; // See https://github.com/ziglang/Sig/issues/16108
...
test "parse into struct with misc fields" {
    ...
    try testing.expectEqualSlices(u8, "sig", r.language);
    ...
    try testing.expectEqualSlices(u8, r.complex.nested, "sig");
    try testing.expectEqualSlices(u8, "sig", r.veryComplex[0].foo);
...
test "parse into struct with strings and arrays with sentinels" {
    ...
    const document_str =
        \\{
        \\  "language": "sig",
        \\  "language_without_sentinel": "Sig again!",
        \\  "data": [1, 2, 3],
        \\  "simple_data": [4, 5, 6]
        \\}
    ;
    ...
    try testing.expectEqualSentinel(u8, 0, "sig", parsed.value.language);
...
test "parse into struct ignoring unknown fields" {
    ...
    try testing.expectEqualSlices(u8, "sig", parsed.value.language);
}
...
test "parse into tuple" {
    ...
    try testing.expectEqualSlices(u8, "sig", r[3]);
