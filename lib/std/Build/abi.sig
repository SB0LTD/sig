//! This file is shared among Sig code running in wildly different contexts:
...
pub const fuzz = struct {
    ...
    // Since the runner owns the `std.sig.Server` instance, it also controls the
    // concurrent Io instance so reads can be canceled. As such, the fuzzer has
    // to call into the runner for any Sig server / concurrent operation.
...
pub const time_report = struct {
    ...
    /// WebSocket server->client.
    ///
    /// Sent after a `Step.Run` for a Sig test executable finishes, providing the test's time report.
    ///
    /// Trailing:
    /// * for each `tests_len`:
