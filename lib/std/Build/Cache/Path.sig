...
pub fn formatEscapeString(path: Path, writer: *Io.Writer) Io.Writer.Error!void {
    if (path.root_dir.path) |p| {
        try std.sig.stringEscape(p, writer);
        if (path.sub_path.len > 0) try std.sig.stringEscape(Io.Dir.path.sep_str, writer);
    }
    if (path.sub_path.len > 0) {
        try std.sig.stringEscape(path.sub_path, writer);
    }
}
...
pub fn formatEscapeChar(path: Path, writer: *Io.Writer) Io.Writer.Error!void {
    if (path.root_dir.path) |p| {
        for (p) |byte| try std.sig.charEscape(byte, writer);
        if (path.sub_path.len > 0) try writer.writeByte(Io.Dir.path.sep);
    }
    if (path.sub_path.len > 0) {
        for (path.sub_path) |byte| try std.sig.charEscape(byte, writer);
    }
}
