//! Tracks metadata of file inputs associated with Sig compiler and build
...
/// A set of strings such as the Sig library directory or project source root, which
...
pub const Path = @import("Cache/Path.sig");
pub const Directory = @import("Cache/Directory.sig");
pub const DepTokenizer = @import("Cache/DepTokenizer.sig");
...
pub const Manifest = struct {
    ...
    pub fn populateFileSystemInputs(man: *Manifest, buf: *std.ArrayList(u8)) Allocator.Error!void {
        assert(@typeInfo(std.sig.Server.Message.PathPrefix).@"enum".field_names.len == man.cache.prefixes_len);
