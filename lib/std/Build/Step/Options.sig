...
fn printType(
    ...
    switch (T) {
        []const []const u8 => {
            if (name) |payload| {
                try out.print(gpa, "pub const {f}: []const []const u8 = ", .{std.sig.fmtId(payload)});
            }
        ...
        []const u8 => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: []const u8 = \"{f}\";", .{
                    std.sig.fmtId(some), std.sig.fmtString(value),
                });
            } else {
                try out.print(gpa, "\"{f}\",", .{std.sig.fmtString(value)});
            }
            return out.appendSlice(gpa, "\n");
        },
        [:0]const u8 => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: [:0]const u8 = \"{f}\";", .{ std.sig.fmtId(some), std.sig.fmtString(value) });
            } else {
                try out.print(gpa, "\"{f}\",", .{std.sig.fmtString(value)});
            }
            return out.appendSlice(gpa, "\n");
        ...
        ?[]const u8 => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: ?[]const u8 = ", .{std.sig.fmtId(some)});
            }

            if (value) |payload| {
                try out.print(gpa, "\"{f}\"", .{std.sig.fmtString(payload)});
            } else {
        ...
        ?[:0]const u8 => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: ?[:0]const u8 = ", .{std.sig.fmtId(some)});
            }
        ...
        std.SemanticVersion => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: @import(\"std\").SemanticVersion = ", .{std.sig.fmtId(some)});
            }
    ...
    switch (@typeInfo(T)) {
        .array => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: {s} = ", .{ std.sig.fmtId(some), @typeName(T) });
            }
        ...
        => {
            if (name) |some| {
                try out.print(gpa, "pub const {f}: {s} = {any};\n", .{ std.sig.fmtId(some), @typeName(T), value });
            } else {
        ...
        .@"enum" => |info| {
            ...
            if (name) |some| {
                try out.print(gpa, "pub const {f}: {f} = .{f};\n", .{
                    std.sig.fmtId(some),
                    std.sig.fmtId(@typeName(T)),
                    std.sig.fmtIdFlags(@tagName(value), .{ .allow_underscore = true, .allow_primitive = true }),
                });
            }
            return;
        },
...
fn printStruct(
    ...
    try out.appendNTimes(gpa, ' ', indent);
    try out.print(gpa, "pub const {f} = ", .{std.sig.fmtId(@typeName(T))});
    ...
    inline for (val.field_names, val.field_types, val.field_attrs) |field_name, field_type, field_attrs| {
        ...
        // If the type name doesn't contains a '.' the type is from Sig builtins.
        if (std.mem.containsAtLeast(u8, type_name, 1, ".")) {
            try out.print(gpa, "    {f}: {f}", .{
                std.sig.fmtIdFlags(field_name, .{ .allow_underscore = true, .allow_primitive = true }),
                std.sig.fmtId(type_name),
            });
        } else {
...
fn printStructValue(
    ...
    if (struct_val.is_tuple) {
        ...
        inline for (struct_val.field_names) |field_name| {
            try out.appendNTimes(gpa, ' ', indent);
            try out.print(gpa, "    .{f} = ", .{
                std.sig.fmtIdFlags(field_name, .{ .allow_primitive = true, .allow_underscore = true }),
            });
...
/// Returns the main artifact of this Build Step which is a Sig source file
