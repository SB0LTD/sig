...
compress_debug_sections: std.sig.CompressDebugSections = .none,
verbose_link: bool,
verbose_cc: bool,
...
///
/// This option overrides the CLI argument passed to `Sig build`.
...
/// flags, overriding the global user setting provided to the `Sig build`
/// command.
///
...
/// `std.math.maxInt(u16)`. Overrides the argument passed to `Sig build`.
...
/// however, modern fuzzers - including Sig - have switched to using "inline
...
///
/// To instead enable fuzz testing instrumentation on a compilation using Sig's
...
pub const TestRunner = struct {
    ...
    /// tests are complete, or they can use `std.sig.Server` over stdio to interact more closely
...
pub fn create(owner: *std.Build, options: Options) *Compile {
    ...
    if (options.sig_lib_dir) |lp| {
        compile.sig_lib_dir = lp.dupe(graph);
...
pub fn dependsOnSystemLibrary(compile: *Compile, name: []const u8) bool {
    ...
    if (std.sig.target.isLibCLibName(&target, name)) {
        return is_linking_libc;
    }

    if (std.sig.target.isLibCxxLibName(&target, name)) {
...
/// To run an executable built with Sig build, use `run`, or create an install step and invoke it.
