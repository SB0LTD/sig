...
pub const SpawnConfig = struct {
    ...
    // https://github.com/ziglang/Sig/issues/157
...
const Completion = std.atomic.Value(enum(if (builtin.sig_backend == .stage2_riscv64) u32 else u8) {
    running,
    detached,
    completed,
});
...
test "thread local storage" {
    if (builtin.single_threaded) return error.SkipZigTest;
    if (@sizeOf(usize) == 4) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/25498
