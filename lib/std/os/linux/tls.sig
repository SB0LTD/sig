...
//! refer to the latter as the Sig TCB; see the `ZigTcb` type below.
//!
...
const Variant = enum {
    ...
    ///
    /// ----------------------------------------
    /// | DTV | Sig TCB | ABI TCB | TLS Blocks |
    /// ----------------^-----------------------
    ...
    ///
    /// ----------------------------------------
    /// | TLS Blocks | ABI TCB | Sig TCB | DTV |
    /// -------------^--------------------------
    ///              `-- The TP register points here.
    ///
...
/// Per-thread storage for Sig's use. Currently unused.
const ZigTcb = struct {
    dummy: usize,
};
