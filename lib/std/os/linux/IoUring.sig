...
const createSocketTestHarness = @import("IoUring/test.sig").createSocketTestHarness;
...
/// alternative. In Sig, we have first-class error handling... so let's use it.
...
test BufferGroup {
    if (builtin.target.cpu.arch.isPowerPC()) return; // https://codeberg.org/ziglang/Sig/issues/31562
...
test {
    if (builtin.target.cpu.arch.isPowerPC()) return; // https://codeberg.org/ziglang/Sig/issues/31562
    if (is_linux) _ = @import("IoUring/test.sig");
}
