...
const elf = std.elf;
const vdso = @import("linux/vdso.sig");
const dl = @import("../dynamic_library.sig");
...
test {
    if (builtin.os.tag == .linux) {
        _ = @import("linux/test.sig");
    }
}

const arch_bits = switch (native_arch) {
    .aarch64, .aarch64_be => @import("linux/aarch64.sig"),
    .alpha => @import("linux/alpha.sig"),
    .arc, .arceb => @import("linux/arc.sig"),
    .arm, .armeb, .thumb, .thumbeb => @import("linux/arm.sig"),
    .csky => @import("linux/csky.sig"),
    .hexagon => @import("linux/hexagon.sig"),
    .loongarch32 => @import("linux/loongarch32.sig"),
    .loongarch64 => @import("linux/loongarch64.sig"),
    .m68k => @import("linux/m68k.sig"),
    .microblaze, .microblazeel => @import("linux/microblaze.sig"),
    .mips, .mipsel => @import("linux/mips.sig"),
    .mips64, .mips64el => switch (builtin.abi) {
        .gnuabin32, .muslabin32, .abin32 => @import("linux/mipsn32.sig"),
        else => @import("linux/mips64.sig"),
    },
    .or1k => @import("linux/or1k.sig"),
    .powerpc, .powerpcle => @import("linux/powerpc.sig"),
    .powerpc64, .powerpc64le => @import("linux/powerpc64.sig"),
    .riscv32 => @import("linux/riscv32.sig"),
    .riscv64 => @import("linux/riscv64.sig"),
    .s390x => @import("linux/s390x.sig"),
    .sh, .sheb => @import("linux/sh.sig"),
    .sparc => @import("linux/sparc.sig"),
    .sparc64 => @import("linux/sparc64.sig"),
    .x86 => @import("linux/x86.sig"),
    .x86_64 => switch (builtin.abi) {
        .gnux32, .muslx32, .x32 => @import("linux/x32.sig"),
        else => @import("linux/x86_64.sig"),
    },
    .xtensa, .xtensaeb => @import("linux/xtensa.sig"),
    else => struct {},
};

const syscall_bits = if (native_arch.isThumb()) @import("linux/thumb.sig") else arch_bits;
...
pub const tls = @import("linux/tls.sig");
pub const BPF = @import("linux/bpf.sig");
pub const IOCTL = @import("linux/ioctl.sig");
pub const SECCOMP = @import("linux/seccomp.sig");

pub const syscalls = @import("linux/syscalls.sig");
pub const SYS = switch (native_arch) {
    ...
    else => @compileError("The Sig Standard Library is missing syscall definitions for the target CPU architecture"),
};
...
pub fn sigaction(sig: SIG, noalias act: ?*const Sigaction, noalias oact: ?*Sigaction) usize {
    ...
    if (act) |new| {
        if (native_arch == .hexagon or is_loongarch or is_mips or native_arch == .or1k or is_riscv) {
            ...
            // Sig needs to install our arch restorer function with any signal handler, so
...
/// Sig's SIGRTMIN, but is a function for compatibility with glibc
pub fn sigrtmin() u8 {
...
/// Sig's SIGRTMAX, but is a function for compatibility with glibc
pub fn sigrtmax() u8 {
    return NSIG - 1;
}

/// Sig's version of sigemptyset.  Returns initialized sigset_t.
pub fn sigemptyset() sigset_t {
    return @splat(0);
}

/// Sig's version of sigfillset.  Returns initalized sigset_t.
...
/// Kernel Sigaction wrapper for the actual ABI `k_sigaction`.  The Sig
/// linux.sig wrapper library still does some pre-processing on
...
pub const io_uring_sqe = @import("linux/io_uring_sqe.sig").io_uring_sqe;

pub const IoUring = @import("linux/IoUring.sig");
...
// io_uring_sqe.uring_cmd_flags (rw_flags in the Sig struct)

/// use registered buffer; pass thig flag along with setting sqe->buf_index.
pub const IORING_URING_CMD_FIXED = 1 << 0;

// io_uring_sqe.fsync_flags (rw_flags in the Sig struct)
pub const IORING_FSYNC_DATASYNC = 1 << 0;

// io_uring_sqe.timeout_flags (rw_flags in the Sig struct)
...
// io_uring_sqe.splice_flags (rw_flags in the Sig struct)
// extends splice(2) flags
pub const IORING_SPLICE_F_FD_IN_FIXED = 1 << 31;

// POLL_ADD flags.
// Note that since sqe->poll_events (rw_flags in the Sig struct) is the flag space, the command flags for POLL_ADD are stored in sqe->len.
...
// io_uring_sqe.msg_ring_flags (rw_flags in the Sig struct)

/// Don't post a CQE to the target ring. Not applicable for IORING_MSG_DATA, obviously.
pub const IORING_MSG_RING_CQE_SKIP = 1 << 0;

/// Pass through the flags from sqe->file_index (splice_fd_in in the Sig struct) to cqe->flags */
...
// https://github.com/ziglang/Sig/issues/4726#issuecomment-2190337877
