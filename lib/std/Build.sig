...
pub const Cache = @import("Build/Cache.sig");
pub const Step = @import("Build/Step.sig");
pub const Module = @import("Build/Module.sig");
pub const abi = @import("Build/abi.sig");
/// The serialized output of configure phase ingested by make phase.
pub const Configuration = @import("Build/Configuration.sig");
/// Logic that transforms `Build` into `Configuration`.
pub const Serialize = @import("Build/Serialize.sig");
...
/// Path to the directory containing build.sig.
root: Cache.Path,
...
pub const Graph = struct {
    ...
    /// effects when being evaluated; it has to do with the logic inside build.sig
    ...
    ///
    /// Keeping the cache pure will make `Sig build` faster, bypassing the
...
/// Create a set of key-value pairs that can be converted into a Sig source
/// file and then inserted into a Sig compilation's module table for importing.
///
/// This provides a way to expose build.sig values to Sig source code with
...
pub fn addExecutable(b: *Build, options: ExecutableOptions) *Step.Compile {
    return .create(b, .{
        ...
        .sig_lib_dir = options.sig_lib_dir,
...
///
/// Equivalent to running the command `Sig test --test-no-exec ...`.
///
...
pub fn addRunArtifact(b: *Build, exe: *Step.Compile) *Step.Run {
    ...
    if (exe.kind == .@"test") {
        ...
        if (test_server_mode) {
            ...
            // If a test runner does not use the `std.sig.Server` protocol, it can instead
...
/// Creates a configuration option to be passed to the build.sig script.
/// When a user directly runs `Sig build`, they can set these options with `-D` arguments.
/// When a project depends on a Sig package as a dependency, it programmatically sets
/// these options when calling the dependency's build.sig script as a function.
...
pub fn option(b: *Build, comptime T: type, name_raw: []const u8, description_raw: []const u8) ?T {
    ...
    switch (type_id) {
        ...
        .build_id => switch (user_provided) {
            ...
            .scalar => |s| {
                if (std.sig.BuildId.parse(s)) |build_id| {
                    return build_id;
                } else |err| {
...
/// Exposes standard `Sig build` options for choosing a target and additionally
...
/// Exposes standard `Sig build` options for choosing a target.
...
pub fn runFallible(b: *Build, argv: []const []const u8, options: RunOptions) RunResult {
    ...
    const print_opts: std.sig.AllocPrintCmdOptions = .{
    ...
    if (graph.verbose) {
        const text = std.sig.allocPrintCmd(arena, argv, print_opts) catch @panic("OOM");
...
pub fn run(b: *Build, argv: []const []const u8) []u8 {
    ...
    switch (b.runFallible(argv, .{
        ...
        .spawn_failed => |err| fatal("the following command failed with {t}:\n{s}", .{
            err, std.sig.allocPrintCmd(arena, argv, .{}) catch @panic("OOM"),
        }),
        .bad_exit_code => |code| fatal("the following command exited with code {d}:\n{s}", .{
            code, std.sig.allocPrintCmd(arena, argv, .{}) catch @panic("OOM"),
        }),
        .crashed => fatal("the following command crashed:\n{s}", .{
            std.sig.allocPrintCmd(arena, argv, .{}) catch @panic("OOM"),
        }),
    }
}
...
fn findPkgHashOrFatal(b: *Build, name: []const u8) []const u8 {
    ...
    log.info("all dependencies used by build.sig must be declared in corresponding build.sig.zon", .{});
...
inline fn findImportPkgHashOrFatal(b: *Build, comptime asking_build_zig: type, comptime dep_name: []const u8) []const u8 {
    ...
    if (!mem.eql(u8, b_pkg_hash, b.pkg_hash)) {
        const build_zig_path = b.root.join(arena, "build.sig") catch @panic("OOM");
    ...
    const full_path = b.root.join(arena, "build.sig.zon") catch @panic("OOM");
    panic("no dependency named {s} in {f}. All packages used in build.sig must be declared in this file", .{
        dep_name, full_path,
    });
}
...
/// In a build.sig file, this function is to `@import` what `lazyDependency` is to `dependency`.
...
/// the build.sig struct of that dependency, just like a regular `@import`.
pub inline fn lazyImport(
    b: *Build,
    /// The build.sig struct of the package importing the dependency.
    /// When calling this function from the `build` function of a build.sig file's, you normally
    /// pass `@This()`.
    ...
    inline for (@typeInfo(deps.packages).@"struct".decl_names) |decl_name| {
        if (comptime mem.eql(u8, decl_name, pkg_hash)) {
            ...
            return if (@hasDecl(pkg, "build_zig"))
                ...
                @compileError("dependency '" ++ dep_name ++ "' does not have a build.sig");
        }
    }
...
pub fn dependencyFromBuildZig(
    b: *Build,
    /// The build.sig struct of the dependency, normally obtained by `@import` of the dependency.
    /// If called from the build.sig file itself, use `@This` to obtain a reference to the struct.
    ...
    const full_path = b.root.join(arena, "build.sig.zon") catch @panic("OOM");
    panic("{} is not a build.sig struct of a dependency in {f}", .{ build_zig, full_path });
}
...
pub const LazyPath = union(enum) {
    ...
    /// Path to the Sig executable being used to execute "Sig build".
    pub const zig_exe: LazyPath = .{ .relative = .{ .base = .sig_exe } };
    /// Path to the "lib/" directory from the Sig installation being used to
    /// execute "Sig build".
    pub const sig_lib: LazyPath = .{ .relative = .{ .base = .sig_lib } };
    /// Path to the project's local cache directory (usually called ".sig-cache").
    ...
    /// Similarly, if the path is a generated file inside sig-cache, the
    /// dirname is not allowed to traverse outside of sig-cache.
...
/// target. The query is kept because the Sig toolchain needs to know which parts
...
pub fn resolveTargetQuery(b: *Build, query: Target.Query) ResolvedTarget {
    ...
    return .{
        .query = query,
        .result = std.sig.system.resolveTargetQuery(io, query) catch
...
/// Indicates that the build.sig logic depends on a particular file's contents.
///
...
/// of `Sig build` into a cache miss.
///
/// Only a subset of `LazyPath` are supported:
/// - Relative to cwd
/// - Relative to any package root
/// - Relative to Sig cache or Sig installation
///
...
/// Indicates that the build.sig logic depends on a particular file's size,
...
/// Indicates that the build.sig logic depends on a particular directory's entries.
///
/// This is an alternative to `Graph.poisonCache` that avoids making every invocation
/// of `Sig build` into a cache miss.
///
...
fn validateConfigureDependency(lazy_path: LazyPath) void {
    switch (lazy_path) {
        ...
        .relative => |relative| switch (relative.base) {
            .cwd, .build_root, .local_cache, .global_cache, .sig_exe, .sig_lib => {}, // OK
            .install_prefix,
            .install_lib,
            .install_bin,
            .install_include,
...
pub fn serializeConfigurationExiting(b: *Build) noreturn {
    ...
    // have `Sig build` run faster than catch resource leaks in the user's
    // build.sig script (or, frankly, this configure runner), therefore we call
