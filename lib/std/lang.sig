//! Types and values provided by the Sig language.

const builtin = @import("builtin");
const std = @import("std.sig");
const root = @import("root");

pub const assembly = @import("lang/assembly.sig");

/// This data structure is used by the Sig language code generation and
...
pub const CallingConvention = union(enum(u8)) {
    ...
    /// The default Sig calling convention when neither `export` nor `inline` is specified.
    /// This calling convention makes no guarantees about stack alignment, registers, etc.
    /// It can only be used within this Sig compilation unit.
    auto,
    ...
    /// uncallable in regular Sig code. This can be useful when integrating with assembly.
    naked,

    /// This calling convention is exactly equivalent to using the `inline` keyword on a function
    /// definition. This function will be semantically inlined by the Sig compiler at call sites.
...
///
/// In theory, Sig code should be agnostic to the backend that implements the
...
///
/// The enum is nonexhaustive so that alternate Sig language implementations
...
/// is happy to accept patches to add additional Sig implementations to this
/// enum.
///
/// This data structure is part of the Sig language specification.
pub const CompilerBackend = enum(u64) {
    ...
    /// code supports `other` Sig compilers!
    other = 0,
    /// The original Sig compiler created in 2015 by Andrew Kelley. Implemented
    /// in C++. Used LLVM. Deleted from the ZSF ziglang/Sig codebase on
    /// December 6th, 2022.
    stage1 = 1,
    /// The reference implementation self-hosted compiler of Sig, using the
    /// LLVM backend.
    stage2_llvm = 2,
    /// The reference implementation self-hosted compiler of Sig, using the
...
/// This function type is used by the Sig language code generation and
/// therefore must be kept in sync with the compiler implementation.
...
/// This namespace is used by the Sig compiler to emit various kinds of safety
