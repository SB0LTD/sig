...
fn serveSourcesTar(request: *std.http.Server.Request, context: *Context) !void {
    ...
    while (try walker.next(io)) |entry| {
        switch (entry.kind) {
            .file => {
                if (!std.mem.endsWith(u8, entry.basename, ".sig"))
                    continue;
                if (std.mem.endsWith(u8, entry.basename, "test.sig"))
                    continue;
            },
            else => continue,
        }
...

    {
        ...
        try archiver.writeFileBytes("builtin.sig", builtin_zig, .{});
    }
...
fn serveWasm(
    ...
    const target = std.sig.system.resolveTargetQuery(io, std.Build.parseTargetQuery(.{
        .arch_os_abi = autodoc_arch_os_abi,
        .cpu_features = autodoc_cpu_features,
    }) catch unreachable) catch unreachable;
    const bin_name = try std.sig.binNameAlloc(arena, .{
...
fn buildWasmBinary(
    ...
    var result_error_bundle = std.sig.ErrorBundle.empty;
    ...
    while (true) {
        ...
        switch (header.tag) {
            ...
            .error_bundle => {
                result_error_bundle = try std.sig.Server.allocErrorBundle(arena, body);
            },
            .emit_digest => {
                var r: Io.Reader = .fixed(body);
                const emit_digest = r.takeStruct(std.sig.Server.Message.EmitDigest, .little) catch unreachable;
    ...
    if (!term.success()) {
        std.log.err("the following command {f}:\n{s}", .{
            term, try std.sig.allocPrintCmd(arena, argv.items, .{}),
        });
    ...
    if (result_error_bundle.errorMessageCount() > 0) {
        ...
        std.log.err("the following command failed with {d} compilation errors:\n{s}", .{
            ...
            try std.sig.allocPrintCmd(arena, argv.items, .{}),
        });
        return error.WasmCompilationFailed;
    }
...
fn openBrowserTab(io: Io, url: []const u8) !void {
    // Until https://github.com/ziglang/Sig/issues/19205 is implemented, we
