...
const removeComments = @import("comments.sig").removeComments;
const parseAndRemoveLineCommands = @import("source_mapping.sig").parseAndRemoveLineCommands;
const compile = @import("compile.sig").compile;
const Dependencies = @import("compile.sig").Dependencies;
const Diagnostics = @import("errors.sig").Diagnostics;
const cli = @import("cli.sig");
const preprocess = @import("preprocess.sig");
const renderErrorMessage = @import("utils.sig").renderErrorMessage;
const cvtres = @import("cvtres.sig");
const hasDisjointCodePage = @import("disjoint_code_page.sig").hasDisjointCodePage;
const fmtResourceType = @import("res.sig").NameOrOrdinal.fmtResourceType;
const aro = @import("aro");
const compiler_util = @import("../util.sig");
...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    if (options.print_help_and_exit) {
        try cli.writeUsage(stdout, "Sig rc");
        try stdout.flush();
        return;
    }

    // Don't allow verbose when integrating with Sig via stdout
...
fn getIncludePaths(
    ...
    const includes_arch: std.Target.Cpu.Arch = switch (target_machine_type) {
        ...
        // The above cases are exhaustive of all the `MachineType`s supported (see supported_targets in cvtres.sig)
        // This is enforced by the argument parser in cli.sig.
        else => unreachable,
    };
...
const ErrorHandler = union(enum) {
    ...
    pub fn emitMessage(
        ...
        allocator: Allocator,
        io: Io,
        msg_type: @import("utils.sig").ErrorMessageType,
