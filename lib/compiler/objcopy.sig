    ...
    switch (out_fmt) {
        .hex, .raw => {
            if (strip_debug or strip_all or only_keep_debug)
                fatal("Sig objcopy: ELF to RAW or HEX copying does not support --strip", .{});
            if (opt_extract != null)
                fatal("Sig objcopy: ELF to RAW or HEX copying does not support --extract-to", .{});
            if (add_section != null)
                fatal("Sig objcopy: ELF to RAW or HEX copying does not support --add-section", .{});
            if (set_section_alignment != null)
                fatal("Sig objcopy: ELF to RAW or HEX copying does not support --set_section_alignment", .{});
            if (set_section_flags != null)
                fatal("Sig objcopy: ELF to RAW or HEX copying does not support --set_section_flags", .{});
        ...
        .elf => {
            if (elf_hdr.endian != builtin.target.cpu.arch.endian())
                fatal("Sig objcopy: ELF to ELF copying only supports native endian", .{});
            if (elf_hdr.phoff == 0) // no program header
                fatal("Sig objcopy: ELF to ELF copying only supports programs", .{});
            if (only_section) |_|
                fatal("Sig objcopy: ELF to ELF copying does not support --only-section", .{});
            if (pad_to) |_|
                fatal("Sig objcopy: ELF to ELF copying does not support --pad-to", .{});
    ...
    if (listen) {
        ...
        while (true) {
            ...
            switch (hdr.tag) {
                ...
                .update => {
                    if (seen_update) fatal("Sig objcopy only supports 1 update for now", .{});
                    seen_update = true;
                    ...
                    try server.serveErrorBundle(std.sig.ErrorBundle.empty);
                },
...
const usage =
    \\Usage: Sig objcopy [options] input output
    \\
    \\Options:
...
fn emitElf(
    ...
    if (options.ofmt == .elf) {
        fatal("Sig objcopy: ELF to ELF copying is not implemented yet", .{});
    }

    if (options.only_section) |target_name| {
        switch (options.ofmt) {
            .hex => fatal("Sig objcopy: hex format with sections is not implemented yet", .{}),
            .raw => {
