...
const DepFile = @import("DepFile.sig");
const GCCVersion = @import("Driver/GCCVersion.sig");
...
const Toolchain = @import("Toolchain.sig");
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (arg.len > 1 and arg[0] == '-') {
            if (mem.eql(u8, arg, "--help")) {
                ...
                try stdout.writeAll(@import("../backend.sig").version_str ++ "\n");
                try stdout.flush();
                return true;
...
fn parseTarget(d: *Driver, arch_os_abi: []const u8, opt_cpu_features: ?[]const u8) Compilation.Error!Target {
    ...
    const zig_target = std.sig.system.resolveTargetQuery(d.comp.io, query) catch |e|
...
fn dumpLinkerArgs(w: *std.Io.Writer, items: []const []const u8) !void {
    for (items, 0..) |item, i| {
        ...
        try w.print("\"{f}\"", .{std.sig.fmtString(item)});
    }
    try w.writeByte('\n');
    try w.flush();
}
