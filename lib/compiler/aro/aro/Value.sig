...
const Interner = @import("../backend.sig").Interner;
const BigIntSpace = Interner.Tag.Int.BigIntSpace;

const annex_g = @import("annex_g.sig");
const Compilation = @import("Compilation.sig");
const QualType = @import("TypeStore.sig").QualType;
...
test "minUnsignedBits" {
    ...
    comp.target = .fromZigTarget(try std.sig.system.resolveTargetQuery(std.testing.io, target_query));
...
pub fn printString(bytes: []const u8, qt: QualType, comp: *const Compilation, w: *std.Io.Writer) std.Io.Writer.Error!void {
    ...
    try w.writeByte('"');
    switch (size) {
        .@"1" => try std.sig.stringEscape(without_null, w),
        .@"2" => {
