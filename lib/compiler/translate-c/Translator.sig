...
const ast = @import("ast.sig");
const ZigNode = ast.Node;
const ZigTag = ZigNode.Tag;
const builtins = @import("builtins.sig");
const helpers = @import("helpers.sig");
const MacroTranslator = @import("MacroTranslator.sig");
const PatternList = @import("PatternList.sig");
const Scope = @import("Scope.sig");
...
pub fn translate(options: Options) mem.Allocator.Error![]u8 {
    ...
    allocating.writer.writeAll(
        \\const __root = @This();
        \\pub const __builtin = @import("std").sig.c_translation.builtins;
        \\pub const __helpers = @import("std").sig.c_translation.helpers;
        \\
...
fn transRecordDecl(t: *Translator, scope: *Scope, record_qt: QualType) Error!void {
    ...
    const init_node = init: {
        ...
        for (record_ty.fields, 0..) |field, field_index| {
            ...
            // values for translated struct fields permits Sig code to comfortably use such an API.
    ...
    const init_node = init: {
        ...
        if (variable.storage_class == .@"extern") {
            if (array_ty != null and array_ty.?.len == .incomplete) {
                ...
                // direct equivalent in Sig. To translate correctly, we'll have to create a C-pointer
...
/// particular field. This approach is necessary because unlike Sig, a C compiler is not
...
/// Produces a Sig AST node by translating a Type, respecting the width, but modifying the signed-ness.
...
/// Signedness of type when translated to Sig.
...
/// If a statement can possibly translate to a Sig assignment (either directly because it's
...
fn transCompoundAssign(
    ...
    // If the result is unused we can try using the equivalent Sig operator
    // without a block
    if (used == .unused) {
    ...
    // Use the equivalent Sig operator if possible.
...
/// Translates compound assignment using the equivalent Sig operator if possible.
...
/// Sig requires a usize argument for pointer arithmetic, so we intCast to isize and then
/// bitcast to usize; pointer wraparound makes the math work.
/// Sig pointer addition is not commutative (unlike C); the pointer operand needs to be on the left.
...
fn transBuiltinCall(
    ...
    const builtin = builtins.map.get(builtin_name) orelse
        return t.fail(error.UnsupportedTranslation, call.builtin_tok, "TODO implement function '{s}' in std.sig.c_builtins", .{builtin_name});
    ...
    for (call.args, arg_nodes, 0..) |c_arg, *zig_arg, i| {
        if (i < func_ty.params.len) {
            ...
            if (zig_arg.isBoolRes() and !func_ty.params[i].qt.is(t.comp, .bool)) {
                ...
                // in Sig we'll need to cast it from bool to u1 (which will safely coerce to c_int).
...
fn transIntLiteral(
    ...
    // to unsigned char, resulting in 0. In order for this to work, we have to emit this Sig code:
...
fn createCharLiteralNode(t: *Translator, narrow: bool, val: u32) TransError!ZigNode {
    return ZigTag.char_literal.create(t.arena, if (narrow)
        try std.fmt.allocPrint(t.arena, "'{f}'", .{std.sig.fmtChar(@as(u8, @intCast(val)))})
    else
...
pub fn createHelperCallNode(t: *Translator, name: std.meta.DeclEnum(std.sig.c_translation.helpers), args_opt: ?[]const ZigNode) !ZigNode {
    if (args_opt) |args| {
...
fn createFlexibleMemberFn(
    ...
    // Use `_self` instead of the conventional `self` to avoid the Sig error
    ...
    // then shadow that declaration, which Sig rejects.
