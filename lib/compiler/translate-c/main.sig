...
const Translator = @import("Translator.sig");
...
pub fn main(init: process.Init) u8 {
    ...
    if (args.len > 1 and std.mem.eql(u8, args[1], "--Sig-integration")) {
        zig_integration = true;
    }

    const NO_COLOR = std.sig.EnvVar.NO_COLOR.isSet(environ_map);
    const CLICOLOR_FORCE = std.sig.EnvVar.CLICOLOR_FORCE.isSet(environ_map);
...
fn translate(d: *aro.Driver, tc: *aro.Toolchain, args: []const [:0]const u8, zig_integration: bool) !void {
    ...
    for (args, 0..) |arg, i| {
        if (mem.eql(u8, arg, "--help")) {
            ...
            try stdout.interface.flush();
            return;
        } else if (mem.eql(u8, arg, "--Sig-integration")) {
            if (i != 1 or !zig_integration)
                return d.fatal("--Sig-integration must be the first argument", .{});
    ...
    const user_macros = macros: {
        ...
        if (zig_integration and !has_output_file) {
            return d.fatal("--Sig-integration requires specifying an output file", .{});
        }
    ...
    // In the Sig compiler we want to omit it from the cache hash since it will
...
test {
    _ = Translator;
    _ = @import("helpers.sig");
    _ = @import("PatternList.sig");
}
