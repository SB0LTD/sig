...
/// Converts the nodes into a Sig Ast.
/// Caller must free the source slice.
pub fn render(gpa: Allocator, nodes: []const Node) !std.sig.Ast {
    var ctx: Context = .{
        .gpa = gpa,
    };
...
const NodeIndex = std.sig.Ast.Node.Index;
const NodeSubRange = std.sig.Ast.Node.SubRange;
const TokenIndex = std.sig.Ast.TokenIndex;
const TokenTag = std.sig.Token.Tag;

const Context = struct {
    gpa: Allocator,
    buf: std.ArrayList(u8) = .empty,
    nodes: std.sig.Ast.NodeList = .empty,
    extra_data: std.ArrayList(u32) = .empty,
    tokens: std.sig.Ast.TokenList = .empty,
    ...
    fn addIdentifier(c: *Context, bytes: []const u8) Allocator.Error!TokenIndex {
        if (std.sig.primitives.isPrimitive(bytes))
            return c.addTokenFmt(.identifier, "@\"{s}\"", .{bytes});
        return c.addTokenFmt(.identifier, "{f}", .{std.sig.fmtId(bytes)});
    }
    ...
    fn addNode(c: *Context, elem: std.sig.Ast.Node) Allocator.Error!NodeIndex {
        const result: NodeIndex = @fromBackingInt(@intCast(c.nodes.len));
        try c.nodes.append(c.gpa, elem);
        return result;
    }

    fn addExtra(c: *Context, extra: anytype) Allocator.Error!std.sig.Ast.ExtraIndex {
        const info = @typeInfo(@TypeOf(extra)).@"struct";
        try c.extra_data.ensureUnusedCapacity(c.gpa, info.field_names.len);
        const result: std.sig.Ast.ExtraIndex = @fromBackingInt(@intCast(c.extra_data.items.len));
        inline for (info.field_names, info.field_types) |field_name, field_type| {
            const data: u32 = switch (field_type) {
                NodeIndex,
                std.sig.Ast.Node.OptionalIndex,
                std.sig.Ast.OptionalTokenIndex,
                std.sig.Ast.ExtraIndex,
    ...
    switch (node.tag()) {
        ...
        .string_slice => {
            ...
            return c.addNode(.{
                ...
                .data = .{ .node_and_extra = .{
                    string,
                    try c.addExtra(std.sig.Ast.Node.Slice{
                        .start = start,
                        .end = end,
                    }),
                } },
        ...
        .fail_decl => {
            ...
            _ = try c.addToken(.l_paren, "(");
            const err_msg_tok = try c.addTokenFmt(.string_literal, "\"{f}\"", .{std.sig.fmtString(payload.mangled)});
        ...
        .@"while" => {
            ...
            if (cont_expr_opt) |cont_expr| {
                return c.addNode(.{
                    ...
                    .data = .{ .node_and_extra = .{
                        cond,
                        try c.addExtra(std.sig.Ast.Node.WhileCont{
                            .cont_expr = cont_expr,
                            .then_expr = body,
                        }),
                    } },
        ...
        .@"if" => {
            ...
            return c.addNode(.{
                ...
                .data = .{ .node_and_extra = .{
                    cond,
                    try c.addExtra(std.sig.Ast.Node.If{
                        .then_expr = then_expr,
                        .else_expr = else_expr,
                    }),
                } },
...
fn renderContainer(c: *Context, node: Node) !NodeIndex {
    ...
    for (payload.fields, 0..) |field, i| {
        const name_tok = try c.addTokenFmt(.identifier, "{f}", .{std.sig.fmtIdFlags(field.name, .{ .allow_primitive = true })});
        _ = try c.addToken(.colon, ":");
        ...
        if (align_expr_opt) |align_expr| {
            if (value_expr_opt) |value_expr| {
                members[i] = try c.addNode(.{
                    ...
                    .data = .{ .node_and_extra = .{
                        type_expr,
                        try c.addExtra(std.sig.Ast.Node.ContainerField{
...
fn renderFieldAccess(c: *Context, lhs: NodeIndex, field_name: []const u8) !NodeIndex {
    return c.addNode(.{
        ...
        .data = .{ .node_and_token = .{
            lhs, try c.addTokenFmt(.identifier, "{f}", .{std.sig.fmtIdFlags(field_name, .{ .allow_primitive = true })}),
        } },
    });
}
...
fn renderNullSentinelArrayType(c: *Context, len: u64, elem_type: Node) !NodeIndex {
    ...
    return c.addNode(.{
        ...
        .data = .{ .node_and_extra = .{
            len_expr,
            try c.addExtra(std.sig.Ast.Node.ArrayTypeSentinel{
...
fn renderPrefixOp(c: *Context, node: Node, tag: std.sig.Ast.Node.Tag, tok_tag: TokenTag, bytes: []const u8) !NodeIndex {
...
fn renderBinOpGrouped(c: *Context, node: Node, tag: std.sig.Ast.Node.Tag, tok_tag: TokenTag, bytes: []const u8) !NodeIndex {
...
fn renderBinOp(c: *Context, node: Node, tag: std.sig.Ast.Node.Tag, tok_tag: TokenTag, bytes: []const u8) !NodeIndex {
    const payload = @as(*Payload.BinOp, @alignCast(@fieldParentPtr("base", node.ptr_otherwise))).data;
    const lhs = try renderNode(c, payload.lhs);
    return c.addNode(.{
        .tag = tag,
...
fn renderVar(c: *Context, node: Node) !NodeIndex {
    ...
    const section_node_opt = if (payload.linksection_string) |some| blk: {
        ...
        const res = try c.addNode(.{
            .tag = .string_literal,
            .main_token = try c.addTokenFmt(.string_literal, "\"{f}\"", .{std.sig.fmtString(some)}),
            .data = undefined,
        });
        _ = try c.addToken(.r_paren, ")");
        break :blk res;
    ...
    if (section_node_opt) |section_node| {
        return c.addNode(.{
            ...
            .data = .{ .extra_and_opt_node = .{
                try c.addExtra(std.sig.Ast.Node.GlobalVarDecl{
        ...
        if (align_node_opt) |align_node| {
            return c.addNode(.{
                ...
                .main_token = mut_tok,
                .data = .{ .extra_and_opt_node = .{
                    try c.addExtra(std.sig.Ast.Node.LocalVarDecl{
                        .type_node = type_node,
    ...
    const fn_proto = try blk: {
        ...
        if (params.items.len < 2)
            break :blk c.addNode(.{
                ...
                .data = .{
                    .extra_and_opt_node = .{
                        try c.addExtra(std.sig.Ast.Node.FnProtoOne{
            ...
            break :blk c.addNode(.{
                ...
                .data = .{
                    .extra_and_opt_node = .{
                        try c.addExtra(std.sig.Ast.Node.FnProto{
