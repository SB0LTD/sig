...
pub const Block = struct {
    ...
    /// be skipped during the intermediate AST -> Sig AST render step.
    ...
    /// is named `mangle(static_local_ + name)` and the Sig variable within the
    ...
    /// is named `mangle(extern_local + name)` and the Sig variable within the
    /// struct keeps the name of the C variable.
