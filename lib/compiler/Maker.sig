...
const Graph = @import("Maker/Graph.sig");
const Step = @import("Maker/Step.sig");
const Watch = @import("Maker/Watch.sig");
const WebServer = @import("Maker/WebServer.sig");
const ScannedConfig = @import("Maker/ScannedConfig.sig");
const PkgConfig = @import("Maker/PkgConfig.sig");
const Fetch = @import("Maker/Fetch.sig");
const Package = @import("Maker/Package.sig");
...
pub fn main(init: process.Init.Minimal) !void {
    ...
    const sig_lib_arg = prefixedArgOrFatal(args, &arg_i, "--Sig-lib=");
    const zig_exe_arg = prefixedArgOrFatal(args, &arg_i, "--Sig=");
    ...
    while (nextArg(args, &arg_i)) |arg| {
        if (mem.startsWith(u8, arg, "-")) {
                ...
                mem.eql(u8, arg, "--release"))
            {
            ...
            } else if (mem.startsWith(u8, arg, "--Sig-lib=")) {
                fatal("--Sig-lib= argument is special and must be first", .{});
                ...
                graph.build_id = std.sig.BuildId.parse(style) catch |err|
    ...
    graph.local_cache_root = if (override_local_cache_dir) |unresolved_path| std.sig.Directories.openUnresolved(
        arena,
        io,
        cwd_path,
        unresolved_path,
    ...
    configure: while (true) {
        ...
        if (configure(&graph, .{
            ...
            if (protocol_server != null) {
                fatal("(Sig build system) TODO send error messages to client when build.sig compilation fails", .{});
            }
            if (watch_flag and can_fs_watch) {
                fatal("(Sig build system) TODO set up fs watching even when build.sig compilation fails", .{});
            } else {
                fatal("(Sig build system) TODO stay running and wait for user to request rebuild even when build.sig compilation fails", .{});
            }
        }
    }
}
    ...
    const configuration_path: Path, var configuration_lock: ?Cache.Lock = cp: while (true) {
        ...
        const config_man: ?*Cache.Manifest = switch (options.cache_poison) {
            .pure, .disallowed, .ignored => m: {
                ...
                // Prevents a `Sig build` from getting a false positive cache hit following
                // a `Sig build --cache-poison=ignored`.
        ...
        {
            {
                ...
                // Create the dependencies.sig file for configurer to
                // obtain via `@import("@dependencies")`.
                {
                    ...
                    const dependencies_zig_path: Path = .{
                        ...
                        .sub_path = try arena.print("o/{s}/dependencies.sig", .{&hex_digest}),
                    };
                    ...
                    atomic_file.file.writeStreamingAll(io, dependencies_source.items) catch |err|
                        fatal("writing dependencies.sig contents: {t}", .{err});
                ...
                {
                    // Add a CliModule for each package's build.sig.
                    ...
                    // Each build.sig module needs access to each of its
                    // dependencies' build.sig modules by name.
                    for (fetches) |f| {
            ...
            const configure_exe_path: Path = if (std.sig.buildExeSubprocess(gpa, io, .{
        ...
        if (!process.can_spawn) {
            fatal("cannot spawn command on {t}: {f}", .{ native_os, @as(std.sig.SubprocessCommand, .{
                .argv = configure_argv,
            }) });
        }
        ...
        if (!term.success()) {
            ...
            fatal("configure command {f}: {f}", .{ term, @as(std.sig.SubprocessCommand, .{
                .argv = configure_argv,
            }) });
        }
...
fn cmdFetch(gpa: Allocator, graph: *Graph, args: []const []const u8) !void {
    ...
    var fixups: std.sig.Ast.Render.Fixups = .{};
    ...
    const new_node_init = try arena.print(
        ...
        \\        }}
    , .{
        std.sig.fmtString(saved_path_or_url),
        std.sig.fmtString(package_hash_slice),
    });

    const new_node_text = try arena.print(".{f} = {s},\n", .{
        std.sig.fmtIdPU(name), new_node_init,
    });
    ...
    if (manifest.dependencies.get(name)) |dep| {
        ...
        if (dep.hash_node.unwrap()) |hash_node| {
            ...
            // https://github.com/ziglang/Sig/issues/21690
        }
...
const usage_fetch =
    ...
    \\  --save                        Add the fetched package to build.sig.zon
    \\  --save=[name]                 Add the fetched package to build.sig.zon as name
    \\  --save-exact                  Add the fetched package to build.sig.zon, storing the URL verbatim
    \\  --save-exact=[name]           Add the fetched package to build.sig.zon as name, storing the URL verbatim
    \\
;
...
const usage_libc =
    ...
    \\    can pass `--libc [file]` for Sig to use it.
    \\
    \\Usage: Sig libc [paths_file]
    \\
    ...
    switch (template) {
        .example => {
            ...
            if (ok_count == template_paths.len) {
                log.info("see `Sig build --help` for a menu of options", .{});
            }
            return process.cleanExit(io);
        },
        .minimal => {
...
fn cmdLibC(gpa: Allocator, graph: *Graph, args: []const []const u8) !void {
    ...
    const target_query = std.sig.parseTargetQueryOrReportFatalError(gpa, .{
        .arch_os_abi = target_arch_os_abi,
    });
...
fn resolveTopLevelSteps(maker: *Maker, step_names: []const []const u8) ![]const Configuration.Step.Index {
    ...
    for (0..step_names.len) |i| {
        ...
        const s = maker.scanned_config.top_level_steps.get(step_name) orelse {
            log.info("to list available steps: Sig build -l", .{});
...
pub fn printErrorMessages(
    ...
    options: std.sig.ErrorBundle.RenderOptions,
    stderr: Io.Terminal,
    error_style: ErrorStyle,
    multiline_errors: MultilineErrors,
) !void {
...
fn fatalWithHint(comptime f: []const u8, args: anytype) noreturn {
    log.info("to access the help menu: Sig build -h", .{});
    fatal(f, args);
}
...
fn serveBSPHandshake(s: *const std.sig.Server) !void {
...
pub fn packagePath(
    ...
    // Currently, neither configurer nor Maker is aware of the standard Sig
...
fn findBuildRoot(arena: Allocator, io: Io, options: FindBuildRootOptions) !BuildRoot {
    ...
    // Search up parent directories until we find build.sig.
        ...
        if (Io.Dir.cwd().access(io, joined_path, .{})) |_| {
            const dir = if (dirname) |d|
                Io.Dir.cwd().openDir(io, d, .{}) catch |err| {
                    fatal("unable to open directory while searching for build.sig file, {q}: {t}", .{ d, err });
                }
            else
                Io.Dir.cwd();
            return .{
...
const Fork = struct {
    path: Path,
    manifest_ast: std.sig.Ast,
    manifest: Package.Manifest,
    error_bundle: std.sig.ErrorBundle.Wip,
    failed: bool,
    ...
    fn loadFallible(io: Io, gpa: Allocator, fork: *Fork, color: Color) !void {
        ...
        var error_bundle: std.sig.ErrorBundle.Wip = undefined;
...
fn loadManifest(
...
) !struct { Package.Manifest, std.sig.Ast } {
    const rng: std.Random.IoSource = .{ .io = io };
    ...
    var ast = try std.sig.Ast.parse(gpa, manifest_bytes, .{ .mode = .zon });
    errdefer ast.deinit(gpa);

    if (ast.errors.len > 0) {
        try std.sig.printAstErrorsToStderr(gpa, io, ast, Package.Manifest.basename, options.color);
        process.exit(2);
    }
...
fn sanitizeExampleName(arena: Allocator, bytes: []const u8) error{OutOfMemory}![]const u8 {
    ...
    if (!std.sig.isValidId(result.items)) return "foo";
