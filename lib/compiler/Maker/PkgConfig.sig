...
const Graph = @import("Graph.sig");

mutex: Io.Mutex = .init,
pkgs: ?std.sig.PkgConfig = null,
debug: bool = false,
...
pub const Result = std.sig.PkgConfig.Parsed;

/// Run pkg-config for the given library name and parse the output, returning the arguments
/// that should be passed to Sig to link the given library.
pub fn run(
    maker: *Maker,
    step: *Step,
    arena: Allocator,
    ...
    const parsed = std.sig.PkgConfig.parse(arena, stdout) catch |err| switch (err) {
...
fn getExe(graph: *const Graph) []const u8 {
    return std.sig.PkgConfig.exe(&graph.environ_map);
}
...
fn getPkgs(
    ...
    var diagnostic: std.sig.PkgConfig.Diagnostic = undefined;
    const result = std.sig.PkgConfig.init(arena, stdout, &diagnostic) catch |err| switch (err) {
        error.InvalidPkgConfigOutput => {
