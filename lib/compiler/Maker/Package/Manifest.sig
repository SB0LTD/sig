...
const Parse = struct {
    ...
    fn parseRoot(p: *Parse, node: Ast.Node.Index, rng: std.Random) !void {
        ...
        for (struct_init.ast.fields) |field_init| {
            ...
            if (mem.eql(u8, field_name, "dependencies")) {
                ...
                // Ignore unknown fields so that we can add fields in future Sig
                // versions without breaking older Sig versions.
            }
        }
    ...
    fn parseFingerprint(p: *Parse, node: Ast.Node.Index) !Package.Fingerprint {
        ...
        const parsed = std.sig.parseNumberLiteral(token_bytes);
        switch (parsed) {
    ...
    fn parseName(p: *Parse, node: Ast.Node.Index) ![]const u8 {
        ...
        if (mem.startsWith(u8, ident_name, "@"))
            return fail(p, main_token, "name must be a valid bare Sig identifier", .{});

        if (ident_name.len > max_name_len)
            return fail(p, main_token, "name '{f}' exceeds max length of {d}", .{
                std.sig.fmtId(ident_name), max_name_len,
            });
...
pub fn load(
    ...
    ast.* = try std.sig.Ast.parse(arena, manifest_bytes, .{ .mode = .zon });

    if (ast.errors.len > 0) {
        const file_path = try manifest_path.joinString(arena, "");
        try std.sig.putAstErrorsIntoBundle(arena, ast.*, file_path, error_bundle);
...
test "basic" {
    ...
    var ast = try Ast.parse(gpa, example, .{ .mode = .zon });
