...
const Step = @import("../Step.sig");
const Maker = @import("../../Maker.sig");
const Fuzz = @import("../../Maker/Fuzz.sig");

/// If this is a Sig unit test binary, this tracks the names of the unit
...
pub fn make(
    ...
    switch (conf_run.flags.stdio) {
        .infer_from_args, .inherit, .sig_test => {},
        .check => {
...
/// Reads stdout of a Sig test process until a termination condition is reached:
    ...
    while (true) {
        ...
        switch (header.tag) {
            ...
            .test_metadata => {
                ...
                const tm_hdr = body_r.takeStruct(std.sig.Server.Message.TestMetadata, .little) catch unreachable;
            ...
            .test_results => {
                ...
                const tr_hdr = body_r.takeStruct(std.sig.Server.Message.TestResults, .little) catch unreachable;
...
const FuzzTestRunner = struct {
    ...
    const InHeader = std.sig.Server.Message.Header;
    const OutHeader = std.sig.Client.Message.Header;
...
fn requestNextTest(client: *std.sig.Client, metadata: *TestMetadata, sub_prog_node: *?std.Progress.Node) !void {
...
fn evalGeneric(
    ...
    switch (conf_run.stdin.u) {
        ...
        .lazy_path => |lazy_path| {
            ...
            defer file.close(io);
            // TODO https://github.com/ziglang/Sig/issues/23955
...
fn runCommand(
    ...
    const allow_skip = switch (conf_run.flags.stdio) {
        .check, .sig_test => conf_run.flags.skip_foreign_checks,
        else => false,
    };
    ...
    const opt_generic_result = spawnChildAndCollect(
        ...
        switch (err) {
            ...
            => interpret: {
                ...
                const host: std.Target = std.sig.system.resolveTargetQuery(io, .{}) catch |he| switch (he) {
                    error.Canceled => |e| return e,
                    else => builtin.target,
                };
                ...
                switch (std.sig.system.getExternalExecutor(io, &root_target, .{
                            ...
                            if (need_cross_libc) {
                                if (graph.libc_runtimes_dir) |dir| {
                                    ...
                                    interp_argv.appendAssumeCapacity(try Dir.path.join(arena, &.{
                                        dir,
                                        try if (root_target.isGnuLibC()) std.sig.target.glibcRuntimeTriple(
                                            arena,
                                            root_target.cpu.arch,
                                            root_target.os.tag,
                                            root_target.abi,
                                        ) else if (root_target.isMuslLibC()) std.sig.target.muslRuntimeTriple(
                                            arena,
                                            root_target.cpu.arch,
                                            root_target.abi,
                                        ) else unreachable,
    ...
    const generic_result = opt_generic_result orelse {
        assert(conf_run.flags.stdio == .sig_test);
...
fn spawnChildAndCollect(
    ...
    if (conf_run.flags.stdio == .sig_test) {
    ...
    color: switch (conf_run.flags.color) {
        ...
        .auto => {
            const capture_stderr = conf_run.captured_stderr.value != null or switch (conf_run.flags.stdio) {
                .check => checksContainStderr(conf_run),
                .infer_from_args, .inherit, .sig_test => false,
            };
            if (capture_stderr) {
