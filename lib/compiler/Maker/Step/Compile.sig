...
const PkgConfig = @import("../PkgConfig.sig");
...
fn lowerZigArgs(
        ...
        for (try getCompileDependencies(arena, &module_graph, conf, compile_index, false)) |dep_compile_index| {
            ...
            for (modules.keys()) |mod_index| {
                ...
                if (cli_named_modules.modules.getIndex(mod_index)) |module_cli_index| {
                    ...
                    // implies the existence of a Sig compilation unit based on
                    // whether there is a root source file. If there is no root
                    // source file, then this is not a Sig compilation unit - it is
    ...
    for (graph.search_prefixes.items) |search_prefix| {
        ...
        // This prevents a warning, that should probably be upgraded to an error in Sig's
    ...
    // pass that to Sig, e.g. via 'Sig build-lib @args.rsp'
    ...
    if (args_length >= 30 * 1024) {
        ...
        // Write the args to sig-cache/args/<SHA256 hash of args> to avoid conflicts with
        // other Sig build commands running in parallel.
...
pub fn rebuildInFuzzMode(
    ...
    step.result_error_bundle = std.sig.ErrorBundle.empty;
