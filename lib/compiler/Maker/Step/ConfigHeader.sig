...
const header_text = "This file was generated by ConfigHeader using the Sig Build System.";
...
pub fn make(
    ...
    // output_dir is sig-cache/o/HASH
    // output_path is libavutil/avconfig.h
    // We want to open directory sig-cache/o/HASH/libavutil/
    // but keep output_dir as sig-cache/o/HASH for -I include
    const out_path: Path = .{
        .root_dir = cache_root,
...
fn renderValueC(conf: *const Configuration, w: *Writer, newline: []const u8, name: []const u8, value: Value.Index) !void {
    switch (value.unpack(conf)) {
        ...
        .string => |string| try w.print("#define {s} \"{f}\"{s}", .{ name, std.sig.fmtString(string), newline }),
    }
}
...
fn renderValueMeson(
    ...
    switch (value.unpack(conf)) {
        ...
        .string => |string| try w.print("#define {s} \"{f}\"", .{ name, std.sig.fmtString(string) }),
    }
}
