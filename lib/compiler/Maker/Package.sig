...
pub const Fetch = @import("Fetch.sig");
pub const Manifest = @import("Package/Manifest.sig");
...
pub const Hash = struct {
    ...
    /// * name is the name field from build.sig.zon, asserted to be at most 32
    ///   bytes and assumed be a valid Sig identifier
    /// * semver is the version field from build.sig.zon, asserted to be at
    ///   most 32 bytes
    ...
    pub fn initPath(sub_path: []const u8, is_global: bool) Hash {
        ...
        _ = std.mem.print(result.bytes[i..], "{x}", .{&bin_digest}) catch unreachable;
        return result;
    }
