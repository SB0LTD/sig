...
//!
//! 1. Check the local Sig package directory to see if the hash already exists.
//!    If so, load, parse, and validate the build.sig.zon file therein, and
...
//!    project local Sig cache. Rename this directory into the local Sig package
//!    directory and goto step 9, skipping step 10.
//! 3. Fetch and unpack a URL into a temporary directory.
//! 4. Load, parse, and validate the build.sig.zon file therein. It is allowed
//!    for the file to be missing, in which case this fetched package is considered
//!    to be a "naked" package.
//! 5. Apply inclusion rules of the build.sig.zon to the temporary directory by
...
//!    directory.
//! 7. Rename the temporary directory into the local Sig package directory. If
//!    the hash already exists, delete the temporary directory and leave the Sig
...
const git = @import("Fetch/git.sig");
const Package = @import("Package.sig");
const Manifest = Package.Manifest;
const ErrorBundle = std.sig.ErrorBundle;

arena: std.heap.ArenaAllocator,
location: Location,
location_tok: std.sig.Ast.TokenIndex,
hash_tok: std.sig.Ast.OptionalTokenIndex,
name_tok: std.sig.Ast.TokenIndex,
...
parent_manifest_ast: ?*const std.sig.Ast,
...
/// Fetch logic notices whether a package has a build.sig file and sets this flag.
has_build_zig: bool,
...
/// The module for this `Fetch` tasks's package, which exposes `build.sig` as
...
cli_module: ?*@import("../Maker.sig").CliModule,
...
pub const LocalStorage = struct {
    cache_root: Path,
    /// Path to "Sig-pkg" inside the package in which the user ran `Sig build`.
    pkg_root: Path,
};
...
pub const JobQueue = struct {
    ...
    /// Creates the dependencies.sig source code for the build runner to obtain
        ...
        for (keys, jq.table.values()) |*hash, fetch| {
            ...
            if (fetch.have_manifest) {
                ...
                for (manifest.dependencies.keys(), manifest.dependencies.values()) |name, dep| {
                    ...
                    try w.print(
                        "            .{{ \"{f}\", \"{f}\" }},\n",
                        .{ std.sig.fmtString(name), std.sig.fmtString(h.toSlice()) },
                    );
                }
    ...
    fn recompressFallible(
        ...
        // However, if we want Sig users to be able to share cached package
...
pub fn run(f: *Fetch) RunError!void {
    ...
    // Check the global Sig package cache to see if the hash already exists. If
    // so, load, parse, and validate the build.sig.zon file therein, and skip
...
fn runResource(
    ...
    const package_sub_path = blk: {
        ...
        // Load, parse, and validate the unpacked build.sig.zon file. It is allowed
    ...
    // Rename the temporary directory into the local Sig package directory. If
    // the hash already exists, delete the temporary directory and leave the
    // Sig package directory untouched as it may be in use. This is done even
...
pub fn computedPackageHash(f: *const Fetch) Package.Hash {
    ...
    // In the future build.sig.zon fields will be added to allow overriding these values
    // for naked tarballs.
    return .init(f.computed_hash.digest, "N", "V", 0xffff, saturated_size);
}

/// `computeHash` gets a free check for the existence of `build.sig`, but when
...
fn queueJobsForDeps(f: *Fetch) RunError!void {
    ...
    // If the package does not have a build.sig.zon file then there are no dependencies.
...
fn fail(f: *Fetch, msg_tok: std.sig.Ast.TokenIndex, msg_str: u32) RunError {
    const eb = &f.error_bundle;
    try eb.addRootErrorMessage(.{
        .msg = msg_str,
        ...
        Io.Dir.rename(
            ...
            error.DirNotEmpty, error.AccessDenied => {
                ...
                tmp_path.root_dir.handle.deleteTree(io, tmp_path.sub_path) catch |er| switch (er) {
                    error.Canceled => |e| return e,
                    // Garbage files leftover in sig-cache/tmp/ is, as they say
...
fn hashFileFallible(io: Io, dir: Io.Dir, hashed_file: *HashedFile) HashedFile.Error!void {
    ...
    switch (hashed_file.kind) {
        .file => {
            ...
            defer file.close(io);
            // Hard-coded false executable bit: https://github.com/ziglang/Sig/issues/17463
            hasher.update(&.{ 0, 0 });
