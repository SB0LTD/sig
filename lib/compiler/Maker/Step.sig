...
const WebServer = @import("WebServer.sig");
const Maker = @import("../Maker.sig");

pub const CheckFile = @import("Step/CheckFile.sig");
pub const Compile = @import("Step/Compile.sig");
pub const ConfigHeader = @import("Step/ConfigHeader.sig");
pub const FindProgram = @import("Step/FindProgram.sig");
pub const Fmt = @import("Step/Fmt.sig");
pub const InstallArtifact = @import("Step/InstallArtifact.sig");
pub const InstallDir = @import("Step/InstallDir.sig");
pub const InstallFile = @import("Step/InstallFile.sig");
pub const ObjCopy = @import("Step/ObjCopy.sig");
pub const Options = @import("Step/Options.sig");
pub const Run = @import("Step/Run.sig");
pub const TranslateC = @import("Step/TranslateC.sig");
pub const UpdateSourceFiles = @import("Step/UpdateSourceFiles.sig");
pub const WriteFile = @import("Step/WriteFile.sig");
...
result_error_bundle: std.sig.ErrorBundle = .empty,
/// Owned by `Maker.gpa`.
    ...
    var start_ts: ?Io.Timestamp = t: {
        ...
        switch (flags.tag) {
            ...
            .run => {
                ...
                if (run_flags.stdio == .sig_test) break :t null;
            },
            else => {},
        }
...
pub fn setFailedCommand(
    ...
    gpa: Allocator,
    argv: []const []const u8,
    options: std.sig.AllocPrintCmdOptions,
) void {
    s.clearFailedCommand(gpa);
    s.result_failed_command = std.sig.allocPrintCmd(gpa, argv, options) catch |err| switch (err) {
        error.OutOfMemory => {
            s.result_oom = true;
            return;
        },
...
/// is the Sig compiler - the same version that compiled the build runner.
///
...
pub fn evalZigProcess(
    ...
    if (s.getZigProcess()) |zp| update: {
        ...
        var exited = false;
        defer if (exited) {
            s.extended.compile.sig_process = null;
            zp.deinit(io);
            gpa.destroy(zp);
        ...
        const result = zigProcessUpdate(step_index, maker, zp, watch) catch |err| switch (err) {
            ...
            else => |e| return s.fail(maker, "Sig child process monitoring failed: {t}", .{e}),
        };
    ...
    }) catch |err| return s.fail(maker, "failed to spawn Sig compiler {s}: {t}", .{ argv[0], err });

    zp.multi_reader.init(gpa, io, zp.multi_reader_buffer.toStreams(), &.{
        zp.child.stdout.?, zp.child.stderr.?,
    });
    if (watch) s.extended.compile.sig_process = zp;
...
fn zigProcessUpdate(step_index: Configuration.Step.Index, maker: *Maker, zp: *ZigProcess, watch: bool) !?Path {
    ...
    while (true) {
        ...
        switch (header.tag) {
            ...
            .error_bundle => {
                s.result_error_bundle = try std.sig.Server.allocErrorBundle(gpa, body);
                // This message indicates the end of the update.
                if (watch) break;
            },
            .emit_digest => {
                const EmitDigest = std.sig.Server.Message.EmitDigest;
            ...
            .file_system_inputs => {
                ...
                while (it.next()) |prefixed_path| {
                    const prefix_index: std.sig.Server.Message.PathPrefix = @fromBackingInt(@intCast(prefixed_path[0] - 1));
            ...
            .time_report => if (maker.web_server) |ws| {
                const TimeReport = std.sig.Server.Message.TimeReport;
...
pub fn getZigProcess(s: *Step) ?*ZigProcess {
    return switch (s.extended) {
        .compile => |*compile| compile.sig_process,
        else => null,
    };
}

fn sendMessage(io: Io, file: Io.File, tag: std.sig.Client.Message.Tag) !void {
    const header: std.sig.Client.Message.Header = .{
        .tag = tag,
        .bytes_len = 0,
    };
