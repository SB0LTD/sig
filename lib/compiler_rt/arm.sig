...
comptime {
    if (!builtin.is_test) {
        if (arch.isArm()) {
            ...
            // floating-point helper functions (single+double-precision reverse subtraction, y – x), see subdf3.sig
...
const __divmodsi4 = @import("int.sig").__divmodsi4;
const __udivmodsi4 = @import("int.sig").__udivmodsi4;
const __divmoddi4 = @import("int.sig").__divmoddi4;
const __udivmoddi4 = @import("int.sig").__udivmoddi4;

extern fn memset(dest: ?*anyopaque, c: i32, n: usize) ?*anyopaque;
...
test "__aeabi_drsub" {
    if (!builtin.cpu.arch.isArm() or builtin.cpu.arch.isThumb()) return error.SkipZigTest;
    if (builtin.cpu.arch == .armeb and builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/22061
