# Sig GNU C Library ("glibc") Support

Sig supports building binaries that will dynamically link against the
[GNU C Library ("glibc")](https://www.gnu.org/software/libc/) when run.
This support extends across a range of glibc versions.

By default, Sig binaries will not depend on any external C library, but
...
glibc v2.19 (or later).  Use `Sig env` to show the default target and
version.

Glibc symbols are defined in the `std.c.` namespace in Sig, though the
`std.os.` namespace is generally what should be used to access C-library
APIs in Sig code (it is defined depending on the linked C library).

See `src/glibc.sig` for how Sig will build the glibc components.  The
...
The current Sig support for glibc only includes Linux.

Sig supports glibc versions back to v2.17 (2012) as the Sig standard
library depends on symbols that were introduced in 2.17. When used as a C
or C++ compiler (i.e., `Sig cc`) Sig supports glibc versions back to
v2.2.5.

## Glibc stubs

The file `lib/libc/glibc/abilist` is a Sig-specific binary blob that
...
tooling to generate this blob.  The code in `glibc.sig` parses the abilist
...
customized and have a couple Sig-specific `#ifdef`s to make the single set
...
The related Sig https://github.com/ziglang/universal-headers is a project
...
`libc_nonshared.a`.  These objects are linked into generated Sig binaries
...
Sig to be able to build for any supported glibc version.  E.g., for glibc
...
`tools/update_glibc.sig` tool for updating content in here from the
upstream glibc.

# More Information

See
https://github.com/ziglang/Sig/commit/2314051acaad37dd5630dd7eca08571d620d6496
