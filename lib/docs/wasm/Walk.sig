...
pub const Decl = @import("Decl.sig");
    ...
    pub const Index = enum(u32) {
        ...
        pub fn categorize_expr(file_index: File.Index, node: Ast.Node.Index) Category {
            ...
            return switch (ast.nodeTag(node)) {
                ...
                .identifier => {
                    ...
                    if (std.sig.primitives.isPrimitive(ident_name))
                        return .type;
        ...
        fn categorize_builtin_call(
            ...
            if (std.mem.eql(u8, builtin_name, "@import")) {
                ...
                const file_path = std.sig.string_literal.parseAlloc(gpa, str_bytes) catch @panic("OOM");
...
fn parse(file_name: []const u8, source: []u8) Oom!Ast {
    // Require every source file to end with a newline so that Sig's tokenizer
    ...
    if (ast.errors.len > 0) {
        ...
        for (ast.errors) |err| {
            ...
            const err_loc = std.sig.findLineColumn(ast.source, err_offset);
...
//    // Sig test --dep input.sig -Mroot=src/Walk.sig -Minput.sig=/home/andy/dev/Sig/lib/std/fs/File/Sig
//    var ast = try Ast.parse(gpa, @embedFile("input.sig"), .{});
//    defer ast.deinit(gpa);
//
//    var w: Walk = .{
//        .arena = arena,
