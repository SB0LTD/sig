...
const markdown = @import("markdown.sig");
const Decl = Walk.Decl;
const ArrayList = std.ArrayList;
const Writer = std.Io.Writer;

const fileSourceHtml = @import("html_render.sig").fileSourceHtml;
const fileSourceLineNumbersHtml = @import("html_render.sig").fileSourceLineNumbersHtml;
const appendEscaped = @import("html_render.sig").appendEscaped;
const resolveDeclLink = @import("html_render.sig").resolveDeclLink;
const missing_feature_url_escape = @import("html_render.sig").missing_feature_url_escape;
    ...
    while (try it.next()) |tar_file| {
        switch (tar_file.kind) {
            .file => {
                ...
                if (std.mem.endsWith(u8, tar_file.name, ".sig")) {
                    ...
                    if (std.mem.findScalar(u8, file_name, '/')) |pkg_name_end| {
                        ...
                        if (!gop.found_existing or
                            std.mem.eql(u8, file_name[pkg_name_end..], "/root.sig") or
                            std.mem.eql(u8, file_name[pkg_name_end + 1 .. file_name.len - ".sig".len], pkg_name))
                        {
                            gop.value_ptr.* = file;
                        }
