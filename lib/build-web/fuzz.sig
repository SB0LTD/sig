            ...
            .file => {
                ...
                if (std.mem.endsWith(u8, tar_file.name, ".sig")) {
                    ...
                    const mod_name, const is_module_root = p: {
                        if (std.mem.find(u8, file_name, "std/")) |i| break :p .{ "std", std.mem.eql(u8, file_name[i + 4 ..], "std.sig") };
                        if (std.mem.endsWith(u8, file_name, "/builtin.sig")) break :p .{ "builtin", true };
                        break :p .{ "root", std.mem.endsWith(u8, file_name, "/root.sig") };
                    };
...
const nsSince = @import("main.sig").nsSince;
const Slice = @import("main.sig").Slice;
const fatal = @import("main.sig").fatal;
const log = std.log;
