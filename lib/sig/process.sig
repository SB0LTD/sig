...
pub const Windows_Argv_Iterator = struct {
    ...
    pub fn skip(self: *Windows_Argv_Iterator) bool {
        return (self.nextWithStrategy(skip_strategy) catch return false) != null;
    }
...
fn spawnWindows(cmd: *const Command_Buffer, options: Spawn_Options) SigError!Child {
    ...
    const wide_len = os.utf8ToWideText(cmdline_buf[0..cmdline_len], &wide_cmdline) orelse
        return error.BufferTooSmall;
    _ = wide_len;
...
pub fn getenv(name: []const u8, buf: []u8) SigError!?[]const u8 {
    if (native_os == .windows) {
        ...
        const name_wide_len = os.utf8ToWideText(name, &name_wide) orelse return error.BufferTooSmall;
        _ = name_wide_len;
