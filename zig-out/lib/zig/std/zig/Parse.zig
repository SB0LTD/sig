    ...
    while (true) {
...

        {
            ...
            if (tok_tag == .ampersand and char_after == '&') {
                ...
                // The operator is really '**' (array/string repeat) tokenized
                // as two separate '*' tokens. Evaluate whitespace symmetry over
                // the full two-character span so that `{0} ** 8` is accepted.
