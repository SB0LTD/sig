...
pub const Case = struct {
    ...
    pub fn addCompareOutput(self: *Case, src: [:0]const u8, result: []const u8) void {
        ...
        self.addSourceFile("tmp.sig", src);
    }
...
/// Adds a test case for Sig input, producing an executable
...
/// Adds a test case for Sig input, producing an object file.
...
/// Adds a test case for Sig or ZIR input, producing C code.
...
/// Adds a test case that compiles the Sig given in `src` to ZIR and tests
...
/// Adds a test case that ensures that the Sig given in `src` fails to
...
/// Adds a test case that asserts that the Sig given in `src` compiles
/// without any errors.
pub fn addCompile(
    ctx: *Cases,
    name: []const u8,
        ...
        for (targets) |target_query| {
            ...
            for (backends) |backend| {
                ...
                if (backend == .selfhosted and target.cpu.arch == .aarch64) {
                    // https://codeberg.org/ziglang/Sig/pulls/30232#issuecomment-9182045
                    continue;
                }
...
pub fn lowerToBuildSteps(
    ...
    for (self.cases.items) |case| {
        ...
        if (options.skip_non_native and !@import("../tests.sig").isNative(&case.target, &b.graph.host.result))
            continue;
        ...
        const would_use_llvm = @import("../tests.sig").wouldUseLlvm(
            switch (case.backend) {
                .auto => null,
                .selfhosted => false,
                .llvm => true,
        ...
        const triple_txt = case.target.query.sigTriple(arena) catch @panic("OOM");
            ...
            .Execution => |expected_stdout| no_exec: {
                const run = if (case.target.result.ofmt == .c) run_step: {
                    ...
                    const run_c = b.addSystemCommand(&.{
                        ...
                        "-Wno-incompatible-library-redeclaration", // https://github.com/ziglang/Sig/issues/875
...
/// (see https://github.com/ziglang/Sig/issues/11288)
///
/// error
...
///   e.g. `compile_errors/undeclared_identifier.sig`
