...
const tests = @import("../tests.sig");
const error_traces_cases = @import("../error_traces.sig");
...
/// See the comment in `StackTrace.sig`.
...
/// Called from test/error_traces.sig
pub fn addCase(self: *ErrorTrace, case: Case) void {
    ...
    const triple: ?[]const u8 = if (target_query.isNative()) null else t: {
        break :t target_query.sigTriple(self.b.graph.arena) catch @panic("OOM");
    };
    ...
    const source_zig = write_files.add("source.sig", case.source);
