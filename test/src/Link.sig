...
pub const Case = struct {
    ...
    /// Verify the results of a `Sig objdump` call against a snapshot, which
    ...
    pub fn verifyObjdump(
        ...
        run_step.addArgs(&.{ ctx.b.graph.sig_exe, "objdump" });
...
fn createModule(self: *const Link, overlay: OverlayOptions) *Build.Module {
    ...
    const mod = self.b.createModule(.{
        ...
        .root_source_file = overlay.sig_source_file orelse rsf: {
            const bytes = overlay.sig_source_bytes orelse break :rsf null;
            const name = self.b.fmt("{s}.sig", .{overlay.name});
