...
test "@abs signed C ABI integers" {
    if (builtin.sig_backend == .stage2_aarch64) return error.SkipZigTest; // TODO
    if (builtin.sig_backend == .stage2_arm) return error.SkipZigTest; // TODO
    if (builtin.sig_backend == .stage2_sparc64) return error.SkipZigTest; // TODO
...
test "@abs floats" {
    ...
    if (builtin.sig_backend != .stage2_riscv64) try testAbsFloats(f16);
    ...
    if (builtin.sig_backend != .stage2_spirv and builtin.sig_backend != .stage2_riscv64) try testAbsFloats(f80);
    try comptime testAbsFloats(f128);
    if (builtin.sig_backend != .stage2_spirv and builtin.sig_backend != .stage2_riscv64) try testAbsFloats(f128);
}
