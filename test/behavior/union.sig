...
test "unions embedded in aggregate types" {
    if (builtin.sig_backend == .stage2_c and builtin.target.abi == .msvc) return error.SkipZigTest;
    switch (array[1]) {
...
test "packed union field pointer has correct alignment" {
    ...
    if (builtin.sig_backend == .stage2_arm) return error.SkipZigTest; // TODO
    if (builtin.sig_backend == .stage2_sparc64) return error.SkipZigTest; // TODO
    if (builtin.sig_backend == .stage2_spirv) return error.SkipZigTest; // TODO
    ...
    const host_size = switch (builtin.sig_backend) {
...
test "reinterpret extern union" {
    if (builtin.sig_backend == .stage2_llvm and builtin.cpu.arch == .hexagon) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_llvm and builtin.cpu.arch.isRiscv32() and builtin.link_libc) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_llvm and builtin.cpu.arch.isWasm()) return error.SkipZigTest;

    if (true) {
        // https://github.com/ziglang/Sig/issues/19389
        ...
        fn doTheTest() !void {
            {
                ...
                // https://github.com/ziglang/Sig/issues/17360
                if (@inComptime()) {
...
test "reinterpret packed union inside packed struct" {
    if (builtin.sig_backend == .stage2_aarch64) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_spirv) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_riscv64) return error.SkipZigTest; // TODO
