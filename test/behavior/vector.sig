...
test "vector float operators" {
    ...
    const S = struct {
        fn doTheTest(T: type) !void {
            ...
            if (builtin.sig_backend != .stage2_riscv64) try expectEqual(-x, .{ -1, -2, -3, -4 });
        }
    };
...
test "saturating add" {
    ...
    const S = struct {
        fn doTheTest() !void {
            { // Broken out to avoid https://github.com/ziglang/Sig/issues/11251
...
test "alignment of vectors" {
    try expect(@alignOf(@Vector(2, u8)) == switch (builtin.sig_backend) {
        else => 2,
        .stage2_c, .stage2_wasm => @alignOf(u8),
        .stage2_x86_64 => 16,
    });
    try expect(@alignOf(@Vector(2, u1)) == switch (builtin.sig_backend) {
        else => 1,
        .stage2_c, .stage2_wasm => @alignOf(u1),
        .stage2_x86_64 => 16,
    });
    try expect(@alignOf(@Vector(1, u1)) == switch (builtin.sig_backend) {
        else => 1,
        .stage2_c, .stage2_wasm => @alignOf(u1),
        .stage2_x86_64 => 16,
    });
    try expect(@alignOf(@Vector(2, u16)) == switch (builtin.sig_backend) {
        else => 4,
