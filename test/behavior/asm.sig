...
comptime {
    if (builtin.sig_backend != .stage2_arm and
        !(builtin.sig_backend == .stage2_c and builtin.os.tag == .windows) and // MSVC doesn't support inline assembly
        is_x86_64_linux)
    {
        asm (
    ...
    } else if (builtin.sig_backend == .stage2_spirv) {
        asm (
...
test "module level assembly" {
    ...
    if (builtin.sig_backend == .stage2_c and builtin.os.tag == .windows) return error.SkipZigTest; // MSVC doesn't support inline assembly
