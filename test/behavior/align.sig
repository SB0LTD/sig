...
test "large alignment of local constant" {
    ...
    if (builtin.sig_backend == .stage2_spirv) return error.SkipZigTest; // flaky
    if (builtin.sig_backend == .stage2_c and builtin.target.abi == .msvc) return error.SkipZigTest;

    if (builtin.sig_backend == .stage2_x86_64 and builtin.os.tag == .windows) {
...
test "align(N) on functions" {
    ...
    if (builtin.sig_backend == .stage2_c) {
        // https://github.com/ziglang/Sig/issues/16845
        return error.SkipZigTest;
    }

    if (builtin.sig_backend == .stage2_c and builtin.os.tag == .windows) {
...
test "comptime alloc alignment" {
    // TODO: it's impossible to test this in Sig today, since comptime vars do not have runtime addresses.
...
test "sub-aligned pointer field access" {
    ...
    // Originally reported at https://github.com/ziglang/Sig/issues/14904
