...
test "simple variadic function" {
    ...
    if (builtin.sig_backend == .stage2_llvm and !builtin.os.tag.isDarwin() and builtin.cpu.arch.isAARCH64()) {
        // https://github.com/ziglang/Sig/issues/14096
        return error.SkipZigTest;
    }
    if (builtin.cpu.arch == .x86_64 and builtin.os.tag == .windows and builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // TODO
    if (builtin.cpu.arch == .s390x and builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/21350
    if (builtin.cpu.arch.isSPARC() and builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/23718
    if (builtin.cpu.arch.isRISCV() and builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/25064
...

    {
        ...
        // Originally reported at https://github.com/ziglang/Sig/issues/16197
...
test "coerce reference to var arg" {
    ...
    // Originally reported at https://github.com/ziglang/Sig/issues/17494
    var a: i32 = 12;
    var b: i32 = 34;
...
test "unused VaList arg" {
    ...
    if (builtin.cpu.arch == .x86_64 and builtin.os.tag == .windows and builtin.sig_backend == .stage2_llvm) {
        // https://github.com/ziglang/Sig/issues/16961
...
test "floating point VaList args" {
    ...
    if (builtin.sig_backend == .stage2_spirv) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_riscv64) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_llvm) return error.SkipZigTest; // https://github.com/ziglang/Sig/issues/16961
