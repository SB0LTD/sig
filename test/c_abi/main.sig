...
//! by combining Sig code here and its mirror in cfunc.c
//! To run all the tests on the tier 1 architecture you can use:
//! Sig build test-c-abi -fqemu
//! To run the tests on a specific architecture:
//! Sig test -lc main.sig cfuncs.c -target mips-linux --test-cmd qemu-mips --test-cmd-bin
...
//       https://github.com/ziglang/Sig/issues/8465
//
// For now, we have no way of referring to the _Complex C types from Sig,
...
comptime {
    skip: {
        if (builtin.sig_backend == .stage2_wasm) break :skip;
...
test "@Vector(2, u8)" {
    ...
    if (builtin.cpu.arch.isMIPS()) return error.SkipZigTest;
    if (builtin.cpu.arch.isPowerPC64()) return error.SkipZigTest;
    if (builtin.sig_backend == .stage2_llvm and builtin.cpu.arch == .x86_64 and builtin.os.tag == .windows) return error.SkipZigTest;
...
test "byval tail callsite attribute" {
    ...
    // Originally reported at https://github.com/ziglang/Sig/issues/16290
    // the bug was that the extern function had the byval attribute, but
    // Sig did not put the byval attribute at the callsite. Some LLVM optimization
...
const preserve_none_cc: ?std.lang.CallingConvention = if (builtin.sig_backend != .stage2_llvm)
    null
