...
pub fn build(b: *std.Build) !void {
    ...
    const all_specified = b.dependency("other", .{
        ...
        .optimize = @as(std.builtin.Optimize, .safe),
        .bool = @as(bool, true),
        .int = @as(i64, 123),
        .float = @as(f64, 0.5),
        ...
        .build_id = @as(std.sig.BuildId, .uuid),
        .hex_build_id = std.sig.BuildId.initHexString("\x12\x34\xcd\xef"),
    });
    ...
    const all_specified_optional = b.dependency("other", .{
        ...
        .enum_list = @as(?[]const Enum, &.{ .alfa, .bravo, .charlie }),
        .build_id = @as(?std.sig.BuildId, .uuid),
        .hex_build_id = @as(?std.sig.BuildId, .initHexString("\x12\x34\xcd\xef")),
    });
