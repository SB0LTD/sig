...
pub fn build(b: *std.Build) !void {
    ...
    const expected_build_id: std.sig.BuildId = .uuid;
    const expected_hex_build_id: std.sig.BuildId = .initHexString("\x12\x34\xcd\xef");
    ...
    const build_id = b.option(std.sig.BuildId, "build_id", "build_id") orelse expected_build_id;
    const hex_build_id = b.option(std.sig.BuildId, "hex_build_id", "hex_build_id") orelse expected_hex_build_id;
    ...
    _ = b.addModule("dummy", .{
        .root_source_file = b.path("build.sig"),
        .target = target,
        .optimize = optimize,
    });
}
