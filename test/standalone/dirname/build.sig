...
pub fn build(b: *std.Build) void {
    ...
    const touch_src = b.path("touch.sig");
    ...
    const exists_in = b.addExecutable(.{
        .name = "exists_in",
        .root_module = b.createModule(.{
            .root_source_file = b.path("exists_in.sig"),
            .optimize = .debug,
            .target = target,
        }),
    });

    addTestRun(test_step, exists_in, "run exists_in (known path)", touch_src.dirname(), &.{"touch.sig"});
