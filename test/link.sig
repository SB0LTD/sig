pub fn addCases(ctx: *LinkContext) void {
    if (ctx.target.result.isMinGW())
        @import("link/mingw.sig").addCases(ctx);
    ...
    if (ctx.includeTest("explicit-extern-lib-name")) |case| {
        // TODO: Lld.sig does not look at explicit inputs to resolve explicit extern lib names
...
const LinkContext = @import("tests.sig").LinkContext;
