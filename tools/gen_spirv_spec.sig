...
const g = @import("spirv/grammar.sig");
...
pub fn main(init: std.process.Init) !void {
    ...
    var tree = try std.sig.Ast.parse(arena, output, .{});

    if (tree.errors.len != 0) {
        try std.sig.printAstErrorsToStderr(arena, io, tree, "", .auto);
        return;
    }

    var zir = try std.sig.AstGen.generate(arena, tree);
...
fn render(
    ...
    try writer.writeAll(
        \\//! This file is auto-generated by tools/gen_spirv_spec.sig.
        \\
        \\const std = @import("std");
        \\
...
const Formatter = struct {
    ...
    fn format(f: Formatter, writer: *std.Io.Writer) std.Io.Writer.Error!void {
        ...
        // make sure that this won't clobber with Sig keywords
        try writer.print("{f}", .{std.sig.fmtId(fw.buffered())});
    }
};
...
fn renderOpcodes(
    ...
    try writer.print("\npub const {f} = enum(u16) {{\n", .{std.sig.fmtId(opcode_type_name)});
    for (instructions_indices) |i| {
        const inst = instructions[i];
        try writer.print("{f} = {},\n", .{ std.sig.fmtId(inst.opname), inst.opcode });
    }
    ...
    if (want_operands) {
        ...
        for (instructions_indices) |i| {
            const inst = instructions[i];
            try writer.print(".{f} => .{f},\n", .{ std.sig.fmtId(inst.opname), formatId(inst.class.?) });
        }
...
fn renderOperand(
    ...
    if (snake_case) {
        ...
        try writer.print("{f}", .{std.sig.fmtId(field_name)});
    }
    ...
    for (parameters, 0..) |param, j| {
        ...
        if (std.mem.startsWith(u8, param.kind, "Id")) {
            ...
            try writer.print("{f}", .{std.sig.fmtId(param.kind)});
        }
...
fn usageAndExit(arg0: []const u8, code: u8) noreturn {
    ...
    w.print(
        \\Usage: {s} <SPIRV-Headers repository path> <path/to/Sig/src/codegen/spirv/extinst.sig.grammar.json>
        \\
        \\Generates Sig bindings for SPIR-V specifications found in the SPIRV-Headers
