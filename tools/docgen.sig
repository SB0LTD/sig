        ...
        switch (token.id) {
            ...
            .bracket_open => {
                ...
                if (mem.eql(u8, tag_name, "nav")) {
                    ...
                    if (mem.eql(u8, source_type_str, "sig")) {
                        source_type = SyntaxBlock.SourceType.sig;
...
fn printSourceBlock(allocator: Allocator, docgen_tokenizer: *Tokenizer, out: *Writer, syntax_block: SyntaxBlock, content: ?[]const u8) !void {
    ...
    switch (syntax_block.source_type) {
        .sig => try tokenizeAndPrint(allocator, docgen_tokenizer, out, syntax_block.source_token),
        else => {
...
fn genHtml(
    ...
    for (toc.nodes) |node| {
        switch (node) {
            ...
            .Builtin => |tok| {
                try out.writeAll("<figure><figcaption class=\"Sig-cap\"><cite>@import(\"builtin\")</cite></figcaption><pre>");
