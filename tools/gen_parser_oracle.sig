//! Example usage:
//! Sig build gen-parser-oracle
//! Sig build check-parser-oracle
...
pub fn main(init: std.process.Init) !void {
    ...
    // Parse the generated Sig code and render it in the canonical format
    var tree = try std.sig.Ast.parse(gpa, generated, .{});
    defer tree.deinit(gpa);

    if (tree.errors.len != 0) {
        // This should never be reached, but helps a lot when debugging this script.
        try std.sig.printAstErrorsToStderr(gpa, io, tree, "generated", .auto);
    ...
    if (check) {
        ...
        if (!mem.eql(u8, current, aw.written())) {
            ...
            std.log.info("Run Sig build gen-parser-oracle to regenerate", .{});
            std.process.exit(1);
        }
    } else {
...
const Generator = struct {
    ...
    /// lib/std/sig/parser_generated_oracle.sig.
    suffix: usize,
        ...
        switch (node.get(g.p)) {
            ...
            .class => |ranges| {
                ...
                for (g.p.getExtra(ranges)) |n| {
                    ...
                    try g.w.writeAll("'");
                    try std.sig.charEscape(range.start, g.w);
                    try g.w.writeAll("'...'");
                    try std.sig.charEscape(range.end, g.w);
                    try g.w.writeAll("',");
                }
                try g.w.print(
...
const Parser = struct {
    ...
    fn parseChar(p: *Parser) ?u8 {
        if (p.eat('\\')) {
            ...
            return switch (c) {
                // Only the escape sequences actually used in the Sig grammar are implemented
                'n' => '\n',
                'r' => '\r',
                't' => '\t',
                '\'' => '\'',
