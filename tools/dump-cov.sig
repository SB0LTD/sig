//! Reads a Sig coverage file and prints human-readable information to stdout,
...
pub fn main(init: std.process.Init) !void {
    ...
    const target_query_str = switch (args.len) {
        ...
        4 => args[3],
            ...
            \\  example: {0s} sig-out/test .sig-cache/v/xxxxxxxx x86_64-linux
        , .{if (args.len == 0) "dump-cov" else args[0]}),
    };

    const target = std.sig.resolveTargetQueryOrFatal(io, try .parse(.{
