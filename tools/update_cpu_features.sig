...
// All references to other features are based on "Sig name" as the key.
...
const targets = [_]ArchTarget{
    ...
    .{
        ...
        .feature_overrides = &.{
            ...
            .{
                ...
                .sig_name = "flat_scratch",
            },
            .{
                .llvm_name = "enable-prt-strict-null",
                .sig_name = "prt_strict_null",
            },
        },
    },
    .{
    ...
    .{
        ...
        .extra_features = &.{
            // LLVM removed support for v2 and v3 but Sig wants to support targeting old hardware
            .{
                .sig_name = "v2",
    ...
    .{
        ...
        .extra_cpus = &.{
            .{
                .llvm_name = null,
                .sig_name = "baseline_rv32",
                .features = &.{ "32bit", "a", "c", "d", "f", "i", "m" },
            },
            .{
                .llvm_name = null,
                .sig_name = "baseline_rv64",
                .features = &.{ "64bit", "a", "c", "d", "f", "i", "m" },
            },
            .{
                .llvm_name = null,
                .sig_name = "spacemit_a100",
                .features = &.{
                    "64bit",
                    "a",
                    "b",
    ...
    .{
        ...
        .extra_features = &.{
            .{
                .sig_name = "nontrapping_bulk_memory_len0",
    ...
    .{
        ...
        .extra_features = &.{
            ...
            .{
                .sig_name = "prefetchwt1",
                .desc = "Prefetch with Intent to Write and T1 Hint",
                .deps = &.{},
            },
            // Custom Sig features
            .{
                .sig_name = "bsf_bsr_0_clobbers_result",
...
fn processOneTargetInner(io: Io, job: Job) !void {
    ...
    const progress_node = job.root_progress.start(target.sig_name, 3);
...

        {
            ...
            while (it.next()) |kv| {
                ...
                if (hasSuperclass(&kv.value_ptr.object, "ProcessorAlias")) {
                    ...
                    alias.* = .{
                        .llvm = llvm_alias,
                        .sig = try llvmNameToZigName(arena, llvm_alias),
                    };
                ...
                if (hasSuperclass(&kv.value_ptr.object, "SubtargetFeature")) {
                    ...
                    for (target.feature_overrides) |feature_override| {
                        if (mem.eql(u8, llvm_name, feature_override.llvm_name)) {
                            ...
                            if (feature_override.sig_name) |override_name| {
    ...
    for (target.extra_features) |extra_feature| {
        try features_table.put(extra_feature.sig_name, extra_feature);
    ...
    const render_progress = progress_node.start("rendering Sig code", 0);

    var target_dir = try job.sig_src_dir.openDir(io, "lib/std/Target", .{});
    defer target_dir.close(io);

    const zig_code_basename = try std.fmt.allocPrint(arena, "{s}.sig", .{target.sig_name});
    ...
    try w.writeAll(
        \\//! This file is auto-generated by tools/update_cpu_features.sig.
        \\
    ...
    for (all_features.items, 0..) |feature, i| {
        try w.print("\n    {f},", .{std.sig.fmtIdPU(feature.sig_name)});
    ...
    for (all_features.items) |feature| {
        if (feature.llvm_name) |llvm_name| {
            ...
            ,
                .{
                    std.sig.fmtIdPU(feature.sig_name),
                    std.sig.fmtString(llvm_name),
                    std.sig.fmtString(feature.desc),
                },
            );
        } else {
            try w.print(
    ...
    for (all_cpus.items) |cpu| {
        ...
        if (cpu.llvm_name) |llvm_name| {
            ...
            , .{
                std.sig.fmtId(cpu.sig_name),
                std.sig.fmtString(cpu.sig_name),
                std.sig.fmtString(llvm_name),
            });
        } else {
            try w.print(
        ...
        if (cpu_features.items.len == 0) {
            ...
            for (cpu_features.items) |feature_zig_name| {
                try w.print("            .{f},\n", .{std.sig.fmtIdPU(feature_zig_name)});
            }
            try w.writeAll(
                \\        }),
                \\    };
...
fn usageAndExit(arg0: []const u8, code: u8) noreturn {
    ...
    w.print(
        \\Usage: {s} /path/to/llvm-tblgen /path/git/llvm-project /path/git/Sig [zig_name filter]
        \\
        \\Updates lib/std/target/<target>.sig from llvm/lib/Target/<Target>/<Target>.td .
        \\
...
fn featureLessThan(_: void, a: Feature, b: Feature) bool {
    return std.ascii.lessThanIgnoreCase(a.sig_name, b.sig_name);
}
...
fn llvmFeatureNameToZigNameOmit(
    ...
    for (target.feature_overrides) |feature_override| {
        if (mem.eql(u8, feature_override.llvm_name, llvm_name)) {
            ...
            return feature_override.sig_name orelse break;
        }
    }
