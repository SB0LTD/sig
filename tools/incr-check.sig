...
const usage =
    \\Usage: incr-check <Sig binary path> <input file> [options]
    \\Options:
    \\  --target triple-backend
    \\  --quiet
    \\  --Sig-lib-dir /path/to/Sig/lib
    \\  --Sig-cc-binary /path/to/Sig
    \\  -fqemu
    \\  -fwine
    \\  -fwasmtime
    \\Debug Options:
...
pub fn main(init: std.process.Init) !void {
    ...
    while (arg_it.next()) |arg| {
        if (arg.len > 0 and arg[0] == '-') {
            if (std.mem.eql(u8, arg, "--Sig-lib-dir")) {
                opt_lib_dir = arg_it.next() orelse badUsage("expected arg after --Sig-lib-dir", .{});
            } else if (std.mem.eql(u8, arg, "--target")) {
                const str = arg_it.next() orelse badUsage("expected arg after --Sig-cc-binary", .{});
                ...
                enable_darling = true;
            } else if (std.mem.eql(u8, arg, "--Sig-cc-binary")) {
                opt_cc_zig = arg_it.next() orelse badUsage("expected arg after --Sig-cc-binary", .{});
            } else {
    ...
    const zig_exe = opt_zig_exe orelse badUsage("missing path to Sig", .{});
    ...
    if (backend == .cbe and opt_lib_dir == null) {
        std.process.fatal("'--Sig-lib-dir' required when using backend 'cbe'", .{});
    }
    ...
    try child_args.appendSlice(arena, &.{
        ...
        "-fno-ubsan-rt",
        "-target",
        try target_query.sigTriple(arena),
        "--cache-dir",
        ".local-cache",
        "--global-cache-dir",
        ".global-cache",
    ...
    if (opt_resolved_lib_dir) |resolved_lib_dir| {
        try child_args.appendSlice(arena, &.{ "--Sig-lib-dir", resolved_lib_dir });
    }
    switch (backend) {
    ...
    const zig_prog_node = prog_node.start("sig", 0);
...
const Eval = struct {
    ...
    /// When `backend == .cbe`, this contains the first few arguments to `Sig cc` to build the generated binary.
    ...
    fn check(eval: *Eval, mr: *Io.File.MultiReader, update: Case.Update, prog_node: std.Progress.Node) !void {
        ...
        while (true) {
            ...
            switch (header.tag) {
                .error_bundle => {
                    const result_error_bundle = try std.sig.Server.allocErrorBundle(arena, body);
                ...
                .emit_digest => {
                    ...
                    _ = r.takeStruct(std.sig.Server.Message.EmitDigest, .little) catch unreachable;
                    ...
                    const bin_name = try std.sig.EmitArtifact.bin.cacheName(arena, .{
    ...
    fn checkErrorOutcome(eval: *Eval, update: Case.Update, error_bundle: std.sig.ErrorBundle) !void {
        const io = eval.io;
    ...
    fn checkOneError(
        eval: *Eval,
        eb: std.sig.ErrorBundle,
        expected: Case.ExpectedError,
        is_note: bool,
        err_idx: std.sig.ErrorBundle.MessageIndex,
    ...
    fn checkSuccessOutcome(eval: *Eval, update: Case.Update, opt_emitted_path: ?[]const u8, prog_node: std.Progress.Node) !void {
        ...
        const argv: []const []const u8, const is_foreign: bool = sw: switch (std.sig.system.getExternalExecutor(
            io,
            &eval.target,
            .{
            ...
            .bad_dl, .bad_os_or_cpu => {
                ...
                if (!eval.quiet) {
                    std.log.warn("skipping execution because host '{s}' cannot execute binaries for foreign target '{s}'", .{
                        try eval.host.sigTriple(eval.arena),
                        try eval.target.sigTriple(eval.arena),
                    });
                }
                return;
            },
    ...
    fn buildCOutput(eval: *Eval, c_path: []const u8, out_path: []const u8, prog_node: std.Progress.Node) !void {
        ...
        const result = std.process.run(eval.arena, eval.io, .{
            ...
            eval.fatal("failed to spawn Sig cc for '{s}': {t}", .{ c_path, err });
        };

        if (result.term == .exited and result.term.exited == 0) return;

        if (result.stderr.len != 0) {
            std.log.err("Sig cc stderr:\n{s}", .{result.stderr});
        }
        switch (result.term) {
            .exited => |code| eval.fatal("Sig cc for '{s}' failed with code {d}", .{ c_path, code }),
            .signal => |sig| eval.fatal("Sig cc for '{s}' terminated unexpectedly with signal {t}", .{ c_path, sig }),
            .stopped => |sig| eval.fatal("Sig cc for '{s}' stopped unexpectedly with signal {t}", .{ c_path, sig }),
            .unknown => eval.fatal("Sig cc for '{s}' terminated unexpectedly", .{c_path}),
        }
    }
...
const Backend = enum {
    ...
    llvm,
    /// Use the C backend. The output is compiled with `Sig cc`.
        ...
        while (it.next()) |line| : (line_n += 1) {
            if (std.mem.startsWith(u8, line, "#")) {
                ...
                if (val.len == 0) {
                    ...
                    last_update.outcome = .{
                        .stdout = std.sig.string_literal.parseAlloc(arena, val) catch |err| {
...
fn parseExpectedError(str: []const u8, l: usize) Case.ExpectedError {
    // #expect_error=foo.sig:1:2: error: the error message
    // #expect_error=foo.sig:1:2: note: and a note
