...
//! architecture. Meanwhile, Sig supports out-of-the-box cross compilation for
//! every target. So the process to create libc headers that Sig ships is to use
//! this tool.
//!
...
//! You'll then have to manually update Sig source repo with these new files.
...
pub fn main(init: std.process.Init) !void {
    ...
    for (libc_targets) |libc_target| {
        const libc_dir = switch (vendor) {
            .glibc => try std.sig.target.glibcRuntimeTriple(arena, libc_target.arch, .linux, libc_target.abi),
            .musl => std.sig.target.muslArchName(libc_target.arch, libc_target.abi),
