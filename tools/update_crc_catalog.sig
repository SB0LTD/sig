...
fn @"i like cheese"(arena: std.mem.Allocator, io: Io, args: []const []const u8) !void {
    ...
    var zig_code_file = try hash_target_dir.createFile(io, "crc.sig", .{});
    ...
    try code_writer.writeAll(
        \\//! This file is auto-generated by tools/update_crc_catalog.sig.
        \\
        \\const builtin = @import("builtin");
        \\
    );

    var zig_test_file = try crc_target_dir.createFile(io, "test.sig", .{});
    ...
    while (try reader.takeDelimiter('\n')) |line| {
        ...
        if (mem.eql(u8, name, "CRC-32/ISCSI")) {
            try code_writer.print(
                ...
                \\    }});
                \\
            , .{ std.sig.fmtId(name), width, poly, init, refin, refout, xorout });
        } else {
            try code_writer.print(
                \\
        ...
        try test_writer.print(
            ...
            \\}}
            \\
        , .{ name, std.sig.fmtId(name), width, check });
    }
