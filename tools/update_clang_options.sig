...
//! When using `Sig cc`, Sig acts as a proxy between the user and Clang. It does not need
//! to understand all the parameters, but it does need to understand some of them, such as
//! the target. This means that Sig must understand when a C command line parameter expects
...
const ClangCliParam = std.sig.ClangCliParam;

const KnownOpt = struct {
    name: []const u8,

    /// Corresponds to stage.sig ClangArgIterator.Kind
    ident: []const u8,
};
...
pub fn main(init: std.process.Init) !void {
    ...
    try stdout.writeAll(
        \\// This file is generated by tools/update_clang_options.sig.
        \\// Sig fmt: off
        \\
    );
    ...
    for (all_objects.items) |obj| {
        ...
        if (std.mem.eql(u8, name, "MT") and syntax == .flag) {
            ...
            element.ze = std.meta.stringToEnum(ClangCliParam.sigEquivalent, ident) orelse fatal("unknown known option: {s}", .{ident});
...
fn printUsage(w: *std.Io.Writer, arg0: []const u8) std.Io.Writer.Error!void {
    try w.print(
        ...
        \\
        \\Prints to stdout Sig code which you can use to replace the file src/clang_options.zon.
        \\
    , .{arg0});
}
